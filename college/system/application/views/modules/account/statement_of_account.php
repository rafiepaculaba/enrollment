    ...
    <tr>
        ...
        <?php  
        	if($account){
        		...
        		$this->semCode	= $account[0]->semCode;
        	}
        	
        	//retrieve subjects
        	$isCompSubj = $this->User_model->retrieveCompSubj($this->idno, $this->yr, $this->semCode);
        	if($isCompSubj) {
        		$this->compSubj = $isCompSubj[0]->ttl;
        	}
...
//        	}
    ...
    <tr>
        <td class="tabDetailViewDL" width="250"><slot>Computer Subject <font style="text-decoration: line-through;"><b>P</b></font></slot></td>
        <td class="tabDetailViewDF" align="right" width="100"><slot><b>
        <?php  
        if ($account) {
        	...
        	$this->db->where('courseID', $row->courseID); 
        	$this->db->where('yrLevel', $row->yrLevel); 
        	$this->db->where('item', "Computer Subject"); 
	        $sch_fee = $this->User_model->retrieveSchool_fees();
	        
	        //retrieve account details
        	$this->db->where('accID', $this->accID);
			$this->db->where('particular', "Computer Subject");
	        $account_detail = $this->User_model->retrieveAccountDetail();
	        if($account_detail){
        ...
        <?php 
//        	if($isCompSubj){
	        	echo number_format($this->compSubj,1); 
//        	}
	    ?> @ 
	        <font style="text-decoration: line-through;"><b>P</b></font>
        <?php 
