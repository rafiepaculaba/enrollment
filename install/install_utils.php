...
function handleDbCreateSugarUser() {
	...
	switch($_SESSION['setup_db_type']) {
		case "mysql":
		    ...
		    /*******************************************************************************************************
			 * Enrollment System: Separate database for Elem, HS, College Level but the same db user and pswd
			 *******************************************************************************************************/
			foreach ($categoryDatabases as $tab) {
    			$query	= "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, DROP, INDEX
						ON `{$tab}`.*
						TO \"{$setup_db_sugarsales_user}\"@\"{$setup_site_host_name}\"
						IDENTIFIED BY '{$setup_db_sugarsales_password}';";
...
function handleDbCreateDatabase() {
	...
	switch($_SESSION['setup_db_type']) {
		case 'mysql':
			 ...
			 *************************************************************************************/
			foreach ($categoryDatabases as $tab) {
			    echo "{$mod_strings['LBL_PERFORM_CREATE_DB_1']} {$tab} {$mod_strings['LBL_PERFORM_CREATE_DB_2']} {$setup_db_host_name}...";
			    
    			$drop = 'DROP DATABASE IF EXISTS '.$tab;
    			@mysql_query($drop, $link);
    			
    			$query = 'CREATE DATABASE `' . $tab . '` CHARACTER SET utf8 COLLATE utf8_general_ci';
    			@mysql_query($query, $link);
    			echo $mod_strings['LBL_PERFORM_DONE'];
			}
