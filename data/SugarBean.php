...
class SugarBean
{
    ...
    function SugarBean()
    {
    	...
    	if(empty($loaded_defs[$this->object_name]))
    	{
    		...
    		if(isset($dictionary[$this->object_name]))
    		{
    			...
    			if(isset($dictionary[$this->object_name]['optimistic_locking']) && 
    					 $dictionary[$this->object_name]['optimistic_locking'])
    			{
	...
	*/
	function save($check_notify = FALSE)
	{
		...
		else
		{
			...
			if($this->new_schema && $this->new_with_id == false)
			{
				$this->id = create_guid();
			}
			$query = "INSERT into ";
		...
		///////////////////////////////////////////////////////////////////////
		////	ASSIGNMENT NOTIFICATION & MEETING/CALL INVITES
		// send assignment notifications AND invites for activities
		if($check_notify) 
		{ // cn: bug 5795 - no invites sent to Contacts
		...
		////	END ASSIGNMENT NOTIFICATION & MEETING/CALL INVITES
		///////////////////////////////////////////////////////////////////////
					...
					if(isset($this->$field)) 
					{
						...
						else 
						{
                            ...
                            if(isset($value['type']) and $value['type']=='int') {
                                //$query .= $field."=".PearDatabase::quote($this->$field);  
				...
				foreach($this->field_defs as $field=>$value)
				{
					...
					if(!isset($value['source']) || $value['source'] == 'db')
					{
						...
						if(isset($this->$field))
						{
                            ...
                            //into an int.
                            if(isset($value['type']) and $value['type']=='int') {
                                //$values[] = PearDatabase::quote($this->$field);
	...
	*/
		...
		foreach($this->field_defs as $fieldDef) 
		{
			...
			{  // is number?
				...
				if(!empty($this->$field)) 
				{
					...
					} elseif(isset($fieldDef['custom_type']) && ($type == 'float' || $type == 'decimal')) {
						// cn: bug XXXXX preserve designated precesion on custom fields
						$customPrecision = strlen($this->$fieldDef['name']) - strpos($this->$fieldDef['name'], '.') - 1;
						$this->$field = format_number($this->$field, $customPrecision, $customPrecision);
					} else {
     ...
     * @param boolean $delted Optional, default true, if set to false deleted filter will not be added.
     * 
     * Internal function, do not override.
	 */
	...
	*/
	...
	/**
	* Tracks the viewing of a detail record.  This leverages get_summary_text() which is object specific.
    * 
    * Internal function, do not override.
  ...
  function build_related_list_where($query, &$template, $where='', $in='', $order_by, $limit='', $row_offset = 0)
  {
		...
		foreach($this->field_defs as $field=>$value) 
		{
			if(isset($this->$field)) 
			{
				...
				if ((!empty($value['type']) && $value['type'] == 'enum') || (!empty($value['custom_type']) && $value['custom_type'] == 'enum')) 
				{
					...
					{
						$return_array[strtoupper($field)] = str_replace('^,^', ', ', $return_array[strtoupper($field)] );
					}
				} 
				else 
				{
    ...
    */
	 ...
	 * @param array $hook_array he array of hooks to call
	 * @param string $event the string for the current event (i.e. before_save)
	 * @param string $arguments an array of arguments that are specific to the event
	 */
