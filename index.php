...
{ // cn: hack for installer error - we ship a blank config.php
	require('config.php');
	...
	// BEGIN: redirect to install.php if doing a manual install of the offline client
	else if(isset($sugar_config['disc_client']) && 
	        $sugar_config['disc_client'] && !isset($sugar_config['dbconfig']))
	{
	    header('Location: install.php');
	    exit ();
	}
	// END: redirect to install.php if doing a manual install of the offline client
}
...
if(!empty($action) && !empty($module)) {
	...
	if(ereg('^printStudentsElem', $action) || ereg('^printStudentsHS', $action) || ereg('^printStudentsCol', $action) || ereg('^Save', $action) || ereg('^Delete', $action) || ereg('^Popup', $action) ||
        ...
        ereg('^Export', $action) || (ereg('^SupportPortal', $action) && ereg('^documentation', $view))) {
    		$skipHeaders = true;
    		if(ereg('^printStudentsElem', $action) || ereg('^printStudentsHS', $action) || ereg('^printStudentsCol', $action) || ereg('^Popup', $action) || ereg('^ChangePassword', $action) || ereg('^Export', $action) || ereg('^SupportPortal', $action))
    			$skipFooters = true;
	}
...
if(empty($_REQUEST['to_pdf']) && empty($_REQUEST['to_csv'])) 
{
    ...
    echo '<script src="include/javascript/yui/container.js?s=' . $sugar_version . '&c=' . $sugar_config['js_custom_version'] . '"></script>';
...
{
    // blumango code
    // all requested pages will be covered in this require files
...
else 
{
	...
	if (ereg('^listStudents', $action)) {
		echo '<link rel="stylesheet" type="text/css" href="themes/Sugar/navigation.css?s=4.5.1g&c=" />
		<link rel="stylesheet" type="text/css" href="themes/Sugar/style.css?s=4.5.1g&c=" />
		<link rel="stylesheet" type="text/css" href="themes/Sugar/colors.sugar.css?s=4.5.1g&c=" />
		<link rel="stylesheet" type="text/css" href="themes/Sugar/fonts.normal.css?s=4.5.1g&c=" />
		
		
		<link rel="stylesheet" type="text/css" href="include/blumango/css/message.css" />
		<link rel="stylesheet" type="text/css" href="include/blumango/css/dimming.css" />
		
		<link href="themes/Sugar/colors.sugar.css?s=4.5.1g&c=" rel="stylesheet" type="text/css" title="color:sugar" />
		<link href="themes/Sugar/colors.blue.css?s=4.5.1g&c=" rel="alternate stylesheet" type="text/css" title="color:blue" />
		<link href="themes/Sugar/colors.green.css?s=4.5.1g&c=" rel="alternate stylesheet" type="text/css" title="color:green" />
		<link href="themes/Sugar/colors.purple.css?s=4.5.1g&c=" rel="alternate stylesheet" type="text/css" title="color:purple" />
		<link href="themes/Sugar/colors.ocher.css?s=4.5.1g&c=" rel="alternate stylesheet" type="text/css" title="color:ocher" />
		<link href="themes/Sugar/fonts.normal.css?s=4.5.1g&c=" rel="stylesheet" type="text/css" title="font:normal" />
		<link href="themes/Sugar/fonts.larger.css?s=4.5.1g&c=" rel="alternate stylesheet" type="text/css" title="font:larger" />
		<link href="themes/Sugar/fonts.largest.css?s=4.5.1g&c=" rel="alternate stylesheet" type="text/css" title="font:largest" />
		<script language="javascript" src="themes/Sugar/menu.js?s=4.5.1g&c="></script>
		
		<script language="javascript" src="themes/Sugar/cookie.js?s=4.5.1g&c="></script>
		<script language="javascript" src="themes/Sugar/style.js?s=4.5.1g&c="></script>
		
		<script language="javascript" src="include/blumango/javascript/ajax.js"></script>
		<script language="javascript" src="include/blumango/javascript/json.js"></script>
	...
	if (ereg('^printStudentsElem', $action) || ereg('^printStudentsHS', $action) || ereg('^printStudentsCol', $action))
		include ('themes/'.$theme.'/printing.html');
    else 
		include ('themes/'.$theme.'/printing.html');
}
...
if(!$skipFooters) 
{
	...
	if($_REQUEST['action'] != 'Login') 
	{
		...
		<table border='0'><tr><td width='40%' align='right'>{$user_mod_strings['LBL_THEME']}&nbsp;</td>";
		echo "<td width='60%' align='left'>
		<select OnChange='location.href=\"index.php?" . str_replace("&print=true", "", $GLOBALS['request_string']) . "&usertheme=\"+this.value' style='width: 120px; font-size: 10px' name='usertheme'>";
	...
	echo "<tr>";
		...
		echo "<td width='100%' align='center' class='copyRight'>";
		
		//echo "<A href='http://www.sugarforge.org' target='_blank'><img style='margin-top: 2px' border='0' width='30' height='30' src='include/images/3globe.png' alt='Powered By EMSS Inc'></a>\n";
		...
		if($sugar_config['calculate_response_time']) 
		{
			...
			if(!empty($sugar_config['show_page_resources'])) 
			{
				...
				echo "External cache (hits/total=ratio) local ({$GLOBALS['external_cache_request_local_hits']}/{$GLOBALS['external_cache_request_local_total']}=".round($GLOBALS['external_cache_request_local_hits']*100/$GLOBALS['external_cache_request_local_total'],0)."%)";
	...
	echo "<tr><td colspan='20' align='center'><div style='display: none'>** 819 forever **</div></td></tr></table>\n";
	}
