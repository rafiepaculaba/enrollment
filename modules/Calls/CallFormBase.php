	...
	if(!empty($focus->users_arr) && is_array($focus->users_arr )) {
		foreach($focus->users_arr as $user_id) {
			...
			if(isset($acceptStatusUsers[$user_id]) && !empty($acceptStatusUsers[$user_id])) {
				$qU  = 'UPDATE calls_users SET calls_users.accept_status = \''.$acceptStatusUsers[$user_id].'\' ';
				$qU .= 'WHERE calls_users.deleted = 0 ';
				$qU .= 'AND calls_users.call_id = \''.$focus->id.'\' ';
				$qU .= 'AND calls_users.user_id = \''.$user_id.'\'';
				$focus->db->query($qU);
			} 
		}
	}
	...
	if(!empty($focus->contacts_arr) && is_array($focus->contacts_arr)) {
		foreach($focus->contacts_arr as $contact_id) {
			...
			if(isset($acceptStatusContacts[$contact_id]) && !empty($acceptStatusContacts[$contact_id])) {
				$qU  = 'UPDATE calls_contacts SET calls_contacts.accept_status = \''.$acceptStatusContacts[$contact_id].'\' ';
				$qU .= 'WHERE calls_contacts.deleted = 0 ';
				$qU .= 'AND calls_contacts.call_id = \''.$focus->id.'\' ';
				$qU .= 'AND calls_contacts.contact_id = \''.$contact_id.'\'';
				$focus->db->query($qU);
			} 
		}
	}
