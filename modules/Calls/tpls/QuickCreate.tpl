...
<td>
	...
	<td class="dataField"><slot>
        ...
        <td nowrap><select name='time_hour_start' tabindex="2">{$TIME_START_HOUR_OPTIONS}</select>{$TIME_SEPARATOR}<select name='time_minute_start' tabindex="2">{$TIME_START_MINUTE_OPTIONS}</select></td>
        {if $TIME_MERIDIEM}
        <td><select name='meridiem' tabindex="2">{$TIME_MERIDIEM}</select></td>
        {/if}
        </tr>
        <tr>
