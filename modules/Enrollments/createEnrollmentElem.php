...
//    $sugar_smarty->assign('schedList', $schedList);
    ...
    echo $sugar_smarty->fetch('modules/Enrollments/templates/createEnrollmentElem.tpl');
...
{
    if (trim($('secID').value)!="") {
        get_data="secID=" + $('secID').value + "&action=doublecheckopenblocksectionelem";
...
function onCheckSchedCode()
{
    if (trim($('schYear').value) != "" && trim($('schedCode').value) != "") {
    	...
    	if (trim($('schYear').value) == "" || trim($('schedCode').value) == "") {
    		if(trim($('schYear').value) == "" && trim($('schedCode').value) != "") {
    			alert("Error: School Year is required!");
    		} else if (trim($('schYear').value) != "" && trim($('schedCode').value) == "") {
    			alert("Error: Sched Code is required!");
    		} else {
    			alert("Error: School Year and Sched Code is required!");    		
    		}
    	}
    }
}
