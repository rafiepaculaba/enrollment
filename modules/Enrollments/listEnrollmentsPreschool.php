...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($_GET['cmdFilter']) {
        ...
        $secID   = $_SESSION[$_GET['module'].'Preschool_secID'];
    }
    ...
    $_SESSION[$_GET['module'].'Preschool_secID']     = $secID;
    ...
    if ($rstatus!="") {
        if (count($conds[0])) {
            $conds[0][' AND enrollments.rstatus']    = " = '$rstatus' ";
        } else {
            $conds[0]['enrollments.rstatus']    = " = '$rstatus' ";
        }
    }
...
//    	$total_rec=0;
    ...
    $main_url="index.php?module=Enrollments&action=listEnrollmentsPreschool&idno=$idno&lname=$lname&fname=$fname&mname=$mname&yrLevel=$yrLevel&schYear=$schYear&semCode=$semCode&secID=$secID&rstatus=$rstatus";
    ...
    $sugar_smarty->assign('listBlockSections', $result );
   	...
   	$yearLevel='<select name="yrLevel" id="yrLevel" onchange="getSections();" >'."\n";
    ...
    echo $sugar_smarty->fetch('modules/Enrollments/templates/listEnrollmentsPreschool.tpl');
} else {
