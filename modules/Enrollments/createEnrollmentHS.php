...
//    $sugar_smarty->assign('schedList', $schedList);
    ...
    echo $sugar_smarty->fetch('modules/Enrollments/templates/createEnrollmentHS.tpl');
...
function setBlockSection(theSecID, theSecName)
{
    secID   = theSecID;
    secName = theSecName;
    get_data="secID=" + secID + "&action=setblocksectionhs";
...
function onCheckEnrollment()
{
    if (trim($('idno').value) != "" && trim($('schYear').value) != "" ) {
        get_data="idno=" + $('idno').value + "&schYear=" + $('schYear').value + "&action=checkenrollmenths";
...
{
    get_data="idno=" + $('idno').value + "&schYear=" + $('schYear').value + "&action=checkenrollmenths";
    ajaxQuery("modules/Enrollments/enrollmentHandler.php",'GET',get_data,"","onnDoubleCheckEnrollmentHandle");
}


function onnDoubleCheckEnrollmentHandle()
{
	if (xmlHttp.readyState==4) {
        // Get the data from the server's response in text format
    	var ret = xmlHttp.responseText; // if ret not empty/0
...
{
    get_data="idno=" + $('idno').value + "&action=getstudentinfohs";
...
{
    if (trim($('secID').value)!="") {
        get_data="secID=" + $('secID').value + "&action=doublecheckopenblocksectionhs";
...
{
    get_data="action=doublecheckopenschedulehs";
...
function onCheckSchedCode()
{
    if (trim($('schYear').value) != "" && trim($('schedCode').value) != "") {
        ...
        get_data="schYear=" + $('schYear').value + "&schedCode=" + $('schedCode').value + "&action=checkschedcodehs";
...
{
    get_data="schedID=" + theSchedID + "&action=checksubjectlevelhs";
...
{
    get_data="schedID=" + theSchedID + "&action=checkopenschedulehs";
...
{
    get_data="schedID=" + theSchedID + "&action=checkconflicths";
...
{
    get_data="schedID=" + theSchedID + "&action=addschedulehs";
...
{
    get_data="idno=" + $('idno').value + "&schYear=" + $('schYear').value + "&yrLevel=" + $('yrLevel').value + "&action=getsectionshs";
...
{
    get_data="schedID=" + schedID + "&action=remschedulehs";
