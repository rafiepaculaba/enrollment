...
$accessCode = $access->getAccessCode("View Preschool Enrollment");
...
if ($access->check_access($current_user->id, $accessCode)) {
    ...
    //Nursery 1, 2 and Kinder 1, 2
    if ($enrollment->yrLevel=='S') {
...
//    $sugar_smarty->assign('yrLevel', $enrollment->yrLevel );
    ...
    if ($enrollment->rstatus && ($enrollment->schYear==$config->getConfig('School Year')) ) {
        ...
        $accessCode = $access->getAccessCode("Edit Preschool Enrollment");
        ...
        if ($enrollment->rstatus==1) {
            // to check if the user has an access in delete
            $accessCode = $access->getAccessCode("Validate Preschool Enrollment");
    ...
    if ($enrollment->rstatus>0) {
	    $accessCode = $access->getAccessCode("Print Preschool Study Load");
	    $sugar_smarty->assign('hasPrint', $access->check_access($current_user->id, $accessCode) );
    }
    echo $sugar_smarty->fetch('modules/Enrollments/templates/viewEnrollmentPreschool.tpl');
...
{
    redirect('index.php?module=Enrollments&action=validateEnrollmentPreschool&enID='+enID);
}
...
function withdrawEnrollment(enID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Enrollments&action=withdrawEnrollmentPreschool&enID='+enID);
}
...
function deleteEnrollment(enID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Enrollments&action=deleteEnrollmentPreschool&enID='+enID);
}
