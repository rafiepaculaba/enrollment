...
/**
 *
 * Filename: EnrollmentDetailPreschool.php
 * Date: 	 June 27, 2008
 * 
	...
	function retrieveAllEnrollmentDetails($where='',$orderby='endID', $sorting='ASC', $offset=0, $limit='')
	{
		...
		try {
    		...
    		foreach ($records as $row) {
    		    ...
    		    // get the fgrade in the form137
    		    $query = "select firstgrade, secondgrade, thirdgrade, fourthgrade, fgrade from form137 where endID='".$row['endID']."' limit 1";
    		    try {
            	    $this->db->beginTransaction();
        		...
        		if (count($records2)) {
        		   $data[$ctr]['form137_fgrade']      = $records2[0]['fgrade'];
        		   $data[$ctr]['form137_firstgrade']  = $records2[0]['firstgrade'];
        		   $data[$ctr]['form137_secondgrade'] = $records2[0]['secondgrade'];
        		   $data[$ctr]['form137_thirdgrade']  = $records2[0]['thirdgrade'];
        		   $data[$ctr]['form137_fourthgrade'] = $records2[0]['fourthgrade'];
        		} else { 
        		   $data[$ctr]['form137_fgrade']      = "";
        		   $data[$ctr]['form137_firstgrade']  = "";
        		   $data[$ctr]['form137_secondgrade'] = "";
        		   $data[$ctr]['form137_thirdgrade']  = "";
        		   $data[$ctr]['form137_fourthgrade'] = "";
        		}
