<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		<td nowrap="nowrap"><h3><img src="themes/Sugar/images/h3Arrow.gif" border="0" height="11" width="11">&nbsp;Preschool Enrollments</h3></td><td width="100%"><img src="include/images/blank.gif" alt="" height="1" width="1"></td>
	</tr>
</table>
<p>
<table class="listView" border="0" cellpadding="0" cellspacing="0" width="100%"><tbody>
	...
	<tr height="20">
	    ...
	    <input type="hidden" name="module" value="Enrollments" />
	    <input type="hidden" name="action" value="listEnrollmentsPreschool" />
		...
		<td scope="col" class="evenListRowS1" nowrap  class="listViewPaginationTdS1"><slot>{$YEARLEVEL}</slot></td>
		<td scope="row" class="evenListRowS1" nowrap  class="listViewPaginationTdS1">
		<select name="secID" id="secID">
        <option value="">----</option>
        {section name=i loop=$listBlockSections}
            <option value="{$listBlockSections[i].secID}" {if $listBlockSections[i].secID eq $secID} selected {/if}>{$listBlockSections[i].secName}</option>
        {/section}
        </select> 
        </td>
		<td scope="row" class="evenListRowS1" nowrap  class="listViewPaginationTdS1">
        ...
        <option value="">----</option>
        <option value="1" {if $rstatus eq "1"} selected {/if}>P</option>
        <option value="2" {if $rstatus eq "2"} selected {/if}>V</option>
        <option value="0" {if $rstatus eq "0"} selected {/if}>W</option>
        </select> 
        </td>
	...
	{if $list neq ""}
    	...
    	<tr onmouseover="setPointer(this, '0', 'over', '#fdfdfd', '#DEEFFF', '');" onmouseout="setPointer(this, '1', 'out', '#fdfdfd', '#DEEFFF', '');" onmousedown="setPointer(this, '0', 'click', '#fdfdfd', '#DEEFFF', '');" height="20">
            ...
            {else}
                class="oddListRowS1" bgcolor="#ffffff" 
            {/if}
            align="left" bgcolor="#fdfdfd" valign="top"><span sugar="sugar0b"><a href="index.php?module=Enrollments&action=viewEnrollmentPreschool&enID={$list[i].enID}" class="listViewTdLinkS1">{$list[i].idno}</a></span></td>
            ...
            {else}
                class="oddListRowS1" bgcolor="#ffffff" 
            {/if}
    		align="left" bgcolor="#fdfdfd" valign="top"><span sugar="sugar0b"><a href="index.php?module=Enrollments&action=viewEnrollmentPreschool&enID={$list[i].enID}" class="listViewTdLinkS1">{$list[i].lname}</a></span></td>
            ...
            {else}
                class="oddListRowS1" bgcolor="#ffffff" 
            {/if}
    		align="left" bgcolor="#fdfdfd" valign="top"><span sugar="sugar0b"><a href="index.php?module=Enrollments&action=viewEnrollmentPreschool&enID={$list[i].enID}" class="listViewTdLinkS1">{$list[i].fname}</a></span></td>
            ...
            {else}
                class="oddListRowS1" bgcolor="#ffffff" 
            {/if}
    		align="left" bgcolor="#fdfdfd" valign="top"><span sugar="sugar0b"><a href="index.php?module=Enrollments&action=viewEnrollmentPreschool&enID={$list[i].enID}" class="listViewTdLinkS1">{$list[i].mname}</a></span></td>
            ...
            {else}
                class="oddListRowS1" bgcolor="#ffffff" 
            {/if}
            align="left" valign="top">{$list[i].secName}</td>
            ...
            {else}
                class="oddListRowS1" bgcolor="#ffffff" 
            {/if}
            align="left" bgcolor="#fdfdfd" valign="top">{if $list[i].rstatus eq 1} P {elseif $list[i].rstatus eq 2} V {elseif $list[i].rstatus eq 0} <font color="red">W</font> {else} &nbsp; {/if}</td>
