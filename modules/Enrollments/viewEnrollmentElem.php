...
if ($access->check_access($current_user->id, $accessCode)) {
    ...
    if ($enrollment->rstatus && ($enrollment->schYear==$config->getConfig('School Year')) ) {
        ...
        $accessCode = $access->getAccessCode("Withdraw Elem Enrollment");
    ...
    if ($enrollment->rstatus>0) {
	    $accessCode = $access->getAccessCode("Print Elem Study Load");
	    $sugar_smarty->assign('hasPrint', $access->check_access($current_user->id, $accessCode) );
    }    
    echo $sugar_smarty->fetch('modules/Enrollments/templates/viewEnrollmentElem.tpl');
...
{
    redirect('index.php?module=Enrollments&action=validateEnrollmentElem&enID='+enID);
}
...
function withdrawEnrollment(enID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Enrollments&action=withdrawEnrollmentElem&enID='+enID);
}
...
function deleteEnrollment(enID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Enrollments&action=deleteEnrollmentElem&enID='+enID);
}
