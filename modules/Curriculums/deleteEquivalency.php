...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($equivalency->eqID) {
        ...
        if ( !empty($equivalency->curID) ) {
            // delete user group record
            if ($equivalency->deleteEquivalency()) {
            ...
            $msg = "Equivalency ID does not exist!";
    	    $sugar_smarty->assign('class', 'errorbox');
        }
    } else {
        $msg = "Error: No Equivalency ID set!";
        $sugar_smarty->assign('class', 'errorbox');
    }
...
<script language="javascript">
	setTimeout("redirect('index.php?module=Curriculums&action=listEquivalency')", 3000);
</script>