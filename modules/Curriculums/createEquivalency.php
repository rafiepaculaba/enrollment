...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    echo $sugar_smarty->fetch('modules/Curriculums/templates/createEquivalency.tpl');
} else {
	...
	if (xmlHttp.readyState==4) {
    	...
    	if (ret) {
			...
			for(c = 0; c < myData.length; c++){
		    	...
		    	if (myData[c].subjCode!=null) {
    				y.text=myData[c].subjCode + " - " + html_entity_decode(myData[c].descTitle) + " (" + myData[c].units + ")";				
...
<script>
addToValidate('frmFuck','curID', '', true, 'Curriculum');
addToValidate('frmFuck','subjID', '', true, 'Subject');
addToValidate('frmFuck','eqSubjID', '', true, 'Equivalent Subject');
