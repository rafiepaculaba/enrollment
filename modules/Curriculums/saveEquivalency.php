...
if ( isset($_POST['theForm']) ) {
    ...
    if (isset($_POST['eqID'])) {
        $equivalency->eqID = $_POST['eqID'];
    }
	
	if ($equivalency->eqID) {
	    $equivalency->curID	   = $_POST['curID'];
	    $equivalency->subjID   = $_POST['subjID'];
	    $equivalency->eqSubjID = $_POST['eqSubjID'];
	    
	    // update department record
	    if ($equivalency->updateEquivalency()) {
	    ...
	    $lastID = $equivalency->getLastID();
	} else {
	    $equivalency->curID	   = $_POST['curID'];
	    $equivalency->subjID   = $_POST['subjID'];
	    $equivalency->eqSubjID = $_POST['eqSubjID'];

	    // new equivalency record
    	if ($equivalency->createEquivalency()) {
...
setTimeout("redirect('index.php?module=Curriculums&action=viewEquivalency&eqID=<?php echo $lastID; ?>')",3000);
</script>