...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	if (empty($_POST['record']) && empty($_POST['dup_checked'])) {
		...
		if(isset($duplicateAccounts)){
			...
			foreach ($duplicateAccounts as $i=>$account)
			{
			...
			$get['return_module'] = '';
			if(!empty($_POST['return_module'])) $get['return_module'] = $_POST['return_module'];
			else $get['return_module'] = 'Accounts';
			$get['return_action'] = ''; 
			if(!empty($_POST['return_action'])) $get['return_action'] = $_POST['return_action'];
			else $get['return_action'] = 'DetailView';
			if(!empty($_POST['return_id'])) $get['popup'] .= $_POST['return_id'];
			if(!empty($_POST['popup'])) $get['return_id'] .= $_POST['popup'];
            ...
            if (!empty($_POST['is_ajax_call']) && $_POST['is_ajax_call'] == '1')
            {
                ...
                $_SESSION['duplicate_get'] = $get;
                header("Location: index.php?module=Accounts&action=ShowDuplicates");
            }
			return null;
		}
	}
