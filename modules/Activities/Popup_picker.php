	...
	function process_page() {
		...
		foreach ($focus_notes_list as $note) {
			 if (!empty($note->date_modified)) {
				$sort_date_time=$timedate->to_db($note->date_modified);
			 }
		...
		foreach($history_list as $activity)
		{
			...
			if (isset($activity['filename'])) {
				$activity_fields['ATTACHMENT'] = "<a href='download.php?id=".$activity['id']."&type=Notes' target='_blank'>".get_image($image_path."attachment","alt='".$activity['filename']."' border='0' align='absmiddle'")."</a>";
   			}
