...
$accessCode = $access->getAccessCode("View Col OR Header");
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ( $orheader ) {
	    ...
	    $sugar_smarty->assign('course', $course->courseCode );
        ...
        if ($currentTerm == $orheader->term ) {
            ...
            // to check if the user has an access in edit
            $accessCode = $access->getAccessCode("Print Col OR Header");
	    ...
	    echo $sugar_smarty->fetch('modules/Payments/templates/viewORHeader.tpl');
	} else {
...
function cancelORHeader(paymentID,schYear,semCode,idno,amount,orno)
{
    reply=confirm("Do you really want to cancel this record?");
    
    if (reply==true)
        redirect('index.php?module=Payments&action=cancelORHeader&paymentID=' + paymentID + '&schYear=' + schYear + '&semCode=' + semCode + '&idno=' + idno + '&amount=' + amount + '&orno=' + orno);
}

function deleteORHeader(paymentID,schYear,semCode,idno,amount,orno)
{
    reply=confirm("Do you really want to cancel this record?");
    
    if (reply==true)
        redirect('index.php?module=Payments&action=deleteORHeader&paymentID=' + paymentID + '&schYear=' + schYear + '&semCode=' + semCode + '&idno=' + idno + '&amount=' + amount + '&orno=' + orno);
}
