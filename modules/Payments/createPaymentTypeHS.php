...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	echo $sugar_smarty->fetch('modules/Payments/templates/createPaymentTypeHS.tpl');
} else {
...
function checkDuplicateHandle() 
{
	if (xmlHttp.readyState==4) {
    	...
    	if (ret=='') {
    		redirect('index.php?module=Payments&action=createPaymentTypeHS&paymentTypeID=<?php echo $_GET['paymentTypeID']; ?>'); // redirect to entry page
    	} else {
	    	if (ret==-1) {
	    		// ID No. duplicate
