...
<a class="current" id="tab_link_Payment_info" href="index.php?module=Payments&action=viewPaymentHS&paymentID={$paymentID}">Payment Form</a>
</li>	
<li id="tab_li_log">
...
<table width="100%" border="0">
  <tr>
    <td>
   <input class="button" name="cmdback" type="button" id="cmdback" value="Back to List" onclick="redirect('index.php?module=Payments&action=listPaymentsHS');" />
    
	{if $hasEdit eq 1 }
    <input class="button" name="cmdedit" type="button" id="cmdedit" value="Edit" onclick="redirect('index.php?module=Payments&action=editPaymentHS&paymentID={$paymentID}');" />
    {/if}
    
	{if $hasCancel eq 1 }
    <input class="button" name="cmdcancel" type="button" id="cmdcancel" value=" Cancel Payment " onclick="cancelPaymentHS('{$paymentID}','{$idno}','{$accID}','{$schYear}','{$termi}','{$amount}');" />
    {/if}
  </tr>
</table>  
