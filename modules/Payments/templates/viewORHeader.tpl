...
<a class="current" id="tab_link_Payment_info" href="index.php?module=Payments&action=viewORHeader&paymentID={$paymentID}">Payment Form</a>
</li>	
<li id="tab_li_log">
...
<table width="100%" border="0">
  <tr>
    <td>
   <input class="button" name="cmdback" type="button" id="cmdback" value="Back to List" onclick="redirect('index.php?module=Payments&action=listORHeader');" />
    
   {if $hasCancel eq 1 }
   {if $rstatus neq 0} 
    <input class="button" name="cmdcancel" type="button" id="cmdcancel" value="  Cancel Payment " onclick="cancelORHeader('{$paymentID}','{$schYear}','{$semCode}','{$idno}','{$amount}','{$orno}');" />
   {/if}
   {/if}
    
   {if $hasDelete eq 1 }
   {if $rstatus eq 0} 
    <input class="button" name="cmddelete" type="button" id="cmddelete" value="  Delete Payment " onclick="deleteORHeader('{$paymentID}','{$schYear}','{$semCode}','{$idno}','{$amount}','{$orno}');" />
   {/if}
   {/if}
    
   {if $hasPrint eq 1 }
    <input class="button" name="cmdprint" type="button" id="cmdprint" value="Print" onclick="popUp('index.php?module=Payments&action=printORHeader&paymentID={$paymentID}&sugar_body_only=1');" />
    {/if}

    {if $hasPrint eq 1 }
    <input class="button" name="cmdprint" type="button" id="cmdprint" value="Split Print" onclick="popUp('index.php?module=Payments&action=printSplitORHeader&paymentID={$paymentID}&sugar_body_only=1');" />
    {/if}
...
<tbody><tr><td>
    ...
    <tr>
        ...
        <td class="tabDetailViewDL" align="right" width="17%"><slot>Semester: </slot></td>
        <td class="tabDetailViewDF" width="33%">{$semCode} &nbsp;</td>
    </tr>
    <tr>
    ...
    <tr>
        <td class="tabDetailViewDL" width="18%"><slot>Course-Year &nbsp;</slot></td>
        <td class="tabDetailViewDF" width="32%">{$course}-{$yrLevel}&nbsp;</td>
