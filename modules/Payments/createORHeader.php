...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    $sugar_smarty->assign('sem_code', $default_semCode);
    ...
    foreach ($theTerms as $key=>$value) {
        if ($currentTerm == $key) {
        	$sugar_smarty->assign('current_term', $key);
        	$sugar_smarty->assign('payment_term', $value);
	...
	echo $sugar_smarty->fetch('modules/Payments/templates/createORHeader.tpl');
...
function oncheckStudentHandle2()
{
	if (xmlHttp.readyState==4) {
    	...
    	if (ret && (ret!='[]') && (ret!='0') && (ret!='')) {
            ...
            $('course').value   = myData[0].course + ' - ' + myData[0].yrLevel;
...
{
    get_data="orno=" + $('orno').value + "&action=checkduplicateorno";
