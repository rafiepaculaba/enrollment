...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    $sugar_smarty->assign('cashier', $orseries->cashier );
    $sugar_smarty->assign('rstatus', $orseries->rstatus );

	
    echo $sugar_smarty->fetch('include/blumango/tpl/message.tpl');
	echo $sugar_smarty->fetch('modules/Payments/templates/editORSeries.tpl');
} else {
...
function saveORSeries() {
    if (check_form('frmORSeries')) {
        if(parseInt($('firstORNO').value) <= parseInt($('lastORNO').value)) {
            if($('firstORNO').value != $('prevfirstORNO').value) {
                checkDuplicate();
            } else {
                $('frmORSeries').submit();
            }
