...
echo $sugar_smarty->fetch('modules/Payments/templates/createRegistrationPaymentHS.tpl');
...
<script>
addToValidate('frmRegistrationPaymentHS','schYear', '', true, 'School Year');
addToValidate('frmRegistrationPaymentHS','idno', '', true, 'ID No.');
addToValidate('frmRegistrationPaymentHS','ORno', '', true, 'Official Receipt No.');
addToValidate('frmRegistrationPaymentHS','date', '', true, 'Date');
addToValidate('frmRegistrationPaymentHS','amount', '', true, 'Amount');
</script>
...
{
    get_data="schYear=" + $('schYear').value + "&idno=" + $('idno').value + "&action=validatepaymenths";
...
{
    get_data="idno=" + $('idno').value + "&action=checkstudinformationhs";
    	...
    	if (ret) {
			...
			if(ret == '[]' || ret == '0' ) {
				if ($('amount').value == '' || $('schYear').value == '' || $('idno').value == '') {
					if (check_form('frmRegistrationPaymentHS')) {
			    				$('frmRegistrationPaymentHS').submit();	
			    	}
				} else {
