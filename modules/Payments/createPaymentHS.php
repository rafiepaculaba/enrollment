...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	//terms
	$terms ='<select name="term" id="term" onchange="displayAccountsHS();" >'."\n";
	...
	echo $sugar_smarty->fetch('modules/Payments/templates/createPaymentHS.tpl');
...
<script>
addToValidate('frmPaymentHS','schYear', '', true, 'School Year');
addToValidate('frmPaymentHS','idno', '', true, 'ID No.');
addToValidate('frmPaymentHS','ORno', '', true, 'Official Receipt No.');
addToValidate('frmPaymentHS','date', '', true, 'Date');
addToValidate('frmPaymentHS','term', '', true, 'Term');
...
{
    get_data="schYear=" + $('schYear').value + "&term=" + $('term').value + "&idno=" + $('idno').value + "&action=validatepaymentpaymenths";
...
{
    get_data="idno=" + $('idno').value + "&action=checkstudinformationpaymenths";
...
function displayStudentInfoHS()
{
    get_data="idno=" + $('idno').value + "&action=displaystudentinfosHS";
    ajaxQuery("modules/Payments/paymentHandler.php",'GET',get_data,"","onDisplayStudentInfosHandleHS");
}

function onDisplayStudentInfosHandleHS()
{
	if (xmlHttp.readyState==4) {
        // Get the data from the server's response in text format
    	var ret = xmlHttp.responseText;
    	if (ret) {
			displayStudentInfoHS2();
			$('assID').value 			= "";
			$('assID_td').innerHTML		= "-";
			$('accID').innerHTML 		= "-";
			$('accID_input').value 		= "";
...
function displayStudentInfoHS2()
{
    get_data="idno=" + $('idno').value + "&action=displaystudentinfosHS";
    ajaxQuery("modules/Payments/paymentHandler.php",'GET',get_data,"","onDisplayStudentInfosHandleHS2");
}

function onDisplayStudentInfosHandleHS2()
{
	if (xmlHttp.readyState==4) {
        // Get the data from the server's response in text format
    	var ret = xmlHttp.responseText;
...
function displayAccountsHS()
{
	if($('term').value != ''){
    get_data="schYear=" + $('schYear').value +"&idno=" + $('idno').value + "&term=" + $('term').value + "&action=displayaccountsHS";
    ajaxQuery("modules/Payments/paymentHandler.php",'GET',get_data,"","onDisplayAccountHandleHS");
	} else {
    			$('assID').value 			= "";
    			$('assID_td').innerHTML		= "-";
    			$('accID').innerHTML 		= "-";
...
function onDisplayAccountHandleHS()
{
	if (xmlHttp.readyState==4) {
        // Get the data from the server's response in text format
    	var ret = xmlHttp.responseText;
