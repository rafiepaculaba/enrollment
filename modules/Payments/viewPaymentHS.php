...
$accessCode = $access->getAccessCode("View HS Payment");
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if($payment->rstatus == 1 && $payment->schYear == $config->getConfig('School Year') ) {
	    ...
	    $accessCode = $access->getAccessCode("Cancel HS Payment");
	    $sugar_smarty->assign('hasCancel', $access->check_access($current_user->id, $accessCode) );
    } 

	echo $sugar_smarty->fetch('modules/Payments/templates/viewPaymentHS.tpl');
	} else {
...
/*function deletePayment(paymentID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Payments&action=deletePaymentHS&paymentID='+paymentID);
}*/
function cancelPaymentHS(paymentID, idno, accID, schYear, term, amount)
{
    reply=confirm("Do you really want to cancel the Payment?");

    if (reply==true)
        redirect('index.php?module=Payments&action=cancelPaymentHS&paymentID='+paymentID+'&idno='+idno+'&accID='+accID+'&schYear='+schYear+'&term='+term+'&amount='+amount);
}
