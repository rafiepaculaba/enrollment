...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	//terms
	$terms ='<select name="term" id="term" onchange="displayAccountsElem();" >'."\n";
	...
	echo $sugar_smarty->fetch('modules/Payments/templates/createPaymentElem.tpl');
...
<script>
addToValidate('frmPaymentElem','schYear', '', true, 'School Year');
addToValidate('frmPaymentElem','idno', '', true, 'ID No.');
addToValidate('frmPaymentElem','ORno', '', true, 'Official Receipt No.');
addToValidate('frmPaymentElem','date', '', true, 'Date');
addToValidate('frmPaymentElem','term', '', true, 'Term');
addToValidate('frmPaymentElem','amount', '', true, 'Amount');
</script>
...
{
    get_data="schYear=" + $('schYear').value + "&term=" + $('term').value + "&idno=" + $('idno').value + "&action=validatepaymentpaymentelem";
...
{
    get_data="idno=" + $('idno').value + "&action=checkstudinformationpaymentelem";
    	...
    	if (ret) {
			...
			if(ret == '[]' || ret == '0' ) {
				if (($('amount').value == '' || $('schYear').value == '' || $('term').value == '' || $('idno').value == '') && ($('assID').value != '')) {
					if (check_form('frmPaymentElem')) {
			    				$('frmPaymentElem').submit();	
			    	}
				} else {
...
function displayStudentInfoElem()
{
    get_data="idno=" + $('idno').value + "&action=displaystudentinfosElem";
    ajaxQuery("modules/Payments/paymentHandler.php",'GET',get_data,"","onDisplayStudentInfosHandleElem");
}

function onDisplayStudentInfosHandleElem()
{
	if (xmlHttp.readyState==4) {
        // Get the data from the server's response in text format
    	var ret = xmlHttp.responseText;
    	if (ret) {
				displayStudentInfoElem2();
    			$('assID').value 			= "";
    			$('assID_td').innerHTML		= "-";
    			$('accID').innerHTML 		= "-";
    			$('accID_input').value		= "";
...
function displayStudentInfoElem2()
{
    get_data="idno=" + $('idno').value + "&action=displaystudentinfosElem";
    ajaxQuery("modules/Payments/paymentHandler.php",'GET',get_data,"","onDisplayStudentInfosHandleElem2");
}

function onDisplayStudentInfosHandleElem2()
{
	if (xmlHttp.readyState==4) {
        // Get the data from the server's response in text format
    	var ret = xmlHttp.responseText;
...
function displayAccountsElem()
{
	if($('term').value != ''){
    get_data="schYear=" + $('schYear').value + "&idno=" + $('idno').value + "&term=" + $('term').value + "&action=displayaccountsElem";
    ajaxQuery("modules/Payments/paymentHandler.php",'GET',get_data,"","onDisplayAccountHandleElem");
	} else {
    			$('assID').value 			= "";
    			$('assID_td').innerHTML		= "-";
    			$('accID').innerHTML 		= "-";
...
function onDisplayAccountHandleElem()
{
	if (xmlHttp.readyState==4) {
        // Get the data from the server's response in text format
    	var ret = xmlHttp.responseText;
