...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	echo $sugar_smarty->fetch('modules/Payments/templates/editPaymentTypeElem.tpl');
} else {
...
<script>
addToValidate('frmPaymentTypeElem','paymentName', '', true, 'Payment Name');
</script>
...
function checkDuplicate()
{
	if($('paymentName').value != $('prevpaymentName').value) {
		if (check_form('frmPaymentTypeElem')) {
	        get_data="paymentName=" + $('paymentName').value + "&action=checkduplicatepaymentnameelem";
	        ajaxQuery("modules/Payments/paymentHandler.php",'GET',get_data,"","checkDuplicateHandle");
	    }
	} else {
			$('frmPaymentTypeElem').submit();
	}
}
...
function checkDuplicateHandle() 
{
	if (xmlHttp.readyState==4) {
    	...
    	if (ret=='') {
    		redirect('index.php?module=Payments&action=editPaymentTypeElem&paymentTypeID=<?php echo $_GET['paymentTypeID']; ?>'); // redirect to entry page
    	} else {
	    	if (ret==-1) {
	    		// ID No. duplicate
                $('frmPaymentTypeElem').submit();
