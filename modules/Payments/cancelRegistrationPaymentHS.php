...
if ($regPayment->regPaymentID) {
    ...
    if ( !empty($regPayment->regPaymentID) ) {
        // cancel registration payment record

        $schYear 	= $_GET['schYear'];
        $idno 		= $_GET['idno'];
		
        //Account and acount instance
		$account 		= new Account();
		...
		$recordLog->docType 		= "HS RegPayment";
...
setTimeout("redirect('index.php?module=Payments&action=viewRegistrationPaymentHS&regPaymentID=<?php echo $regPaymentID ?>')", 3000);
</script>