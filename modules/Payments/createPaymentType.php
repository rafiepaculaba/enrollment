...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	echo $sugar_smarty->fetch('modules/Payments/templates/createPaymentType.tpl');
} else {
...
<script>
addToValidate('frmPaymentType','paymentName', '', true, 'Payment Name');
</script>

<script language="javascript">
function checkDuplicate()
{
    if (check_form('frmPaymentType')) {
        get_data="paymentName=" + $('paymentName').value + "&action=checkduplicatepaymentnamecol";
...
function checkDuplicateHandle() 
{
	if (xmlHttp.readyState==4) {
    	...
    	if (ret=='') {
    		redirect('index.php?module=Payments&action=createPaymentType&paymentTypeID=<?php echo $_GET['paymentTypeID']; ?>'); // redirect to entry page
    	} else {
	    	if (ret==-1) {
	    		// ID No. duplicate
