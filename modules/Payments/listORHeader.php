...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ($_GET['cmdFilter']) {
		...
		$gender       = $_SESSION[$_GET['module'].'Col_gender'];
		$schYear      = $_SESSION[$_GET['module'].'Col_schYear'];
		$semCode      = $_SESSION[$_GET['module'].'Col_semCode'];
		$term         = $_SESSION[$_GET['module'].'Col_term'];
		$dateCreated  = $_SESSION[$_GET['module'].'Col_dateCreated'];
		$totalAmount  = $_SESSION[$_GET['module'].'Col_totalAmount'];
	...
	$_SESSION[$_GET['module'].'Col_gender']        = $gender;
    ...
    if (!isset($_SESSION[$_GET['module'].'Col_rstatus'])) {
        // get default status
        $rstatus = 1;
    }
...
//	$sugar_smarty->assign('user_list', $user_list);
    ...
    if ($semCode) {
       if (count($conds[0])) {
           $conds[0][' AND orheader.semCode'] = " = '$semCode' ";
       } else {
           $conds[0]['orheader.semCode'] = " = '$semCode' ";
       }
    }
	...
	$main_url="index.php?module=Payments&action=listORHeader&paymentID=$paymentID&orno=$orno&idno=$idno&schYear=$schYear&semCode=$semCode&rstatus=$rstatus";
...
//	$sugar_smarty->assign('user_list', $user_list);
	...
	echo $sugar_smarty->fetch('modules/Payments/templates/listORHeader.tpl');
} else {
