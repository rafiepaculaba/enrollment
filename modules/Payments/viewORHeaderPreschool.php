...
$accessCode = $access->getAccessCode("View Preschool OR Header");
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ( $orheader ) {
        ...
        if ($currentTerm == $orheader->term ) {
            ...
            $accessCode = $access->getAccessCode("Edit Preschool OR Header");
            ...
            // to check if the user has an access in edit
            $accessCode = $access->getAccessCode("Print Preschool OR Header");
            $sugar_smarty->assign('hasPrint', $access->check_access($current_user->id, $accessCode) );

            // to check if the user has an access in edit
            $accessCode = $access->getAccessCode("Delete Preschool OR Header");
            $sugar_smarty->assign('hasDelete', $access->check_access($current_user->id, $accessCode) );

        }

	    echo $sugar_smarty->fetch('modules/Payments/templates/viewORHeaderPreschool.tpl');
	} else {
...
function cancelORHeader(paymentID, schYear, idno, amount, orno)
{
    ...
    if (reply==true)
        redirect('index.php?module=Payments&action=cancelORHeaderPreschool&paymentID=' + paymentID + '&schYear=' + schYear + '&idno=' + idno + '&amount=' + amount + '&orno=' + orno);
}
...
function deleteORHeader(paymentID, schYear, idno, amount, orno)
{
    ...
    if (reply==true)
        redirect('index.php?module=Payments&action=deleteORHeaderPreschool&paymentID=' + paymentID + '&schYear=' + schYear + '&idno=' + idno + '&amount=' + amount + '&orno=' + orno);
}
