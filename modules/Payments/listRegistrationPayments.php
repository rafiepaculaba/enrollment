...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($_GET['cmdFilter']) {
    	...
    	$regPaymentID     	= $_SESSION[$_GET['module'].'regCol_regPaymentID'];
    	$schYear     		= $_SESSION[$_GET['module'].'regCol_schYear'];
    	$semCode     		= $_SESSION[$_GET['module'].'regCol_semCode'];
    	$ORno     			= $_SESSION[$_GET['module'].'regCol_ORno'];
    	$type     			= $_SESSION[$_GET['module'].'regCol_type'];
    	$idno     			= $_SESSION[$_GET['module'].'regCol_idno'];
    	$lname     			= $_SESSION[$_GET['module'].'regCol_lname'];
    	$fname     			= $_SESSION[$_GET['module'].'regCol_fname'];
    	$mname     			= $_SESSION[$_GET['module'].'regCol_mname'];
    	$rstatus     		= $_SESSION[$_GET['module'].'regCol_rstatus'];
    }
    
    if (!isset($_GET['schYear']) && !isset($_SESSION[$_GET['module'].'regCol_schYear'])) {
        // get the default schYear
        $schYear = $config->getConfig('School Year');
    }
    
    if (!isset($_GET['semCode']) && !isset($_SESSION[$_GET['module'].'regCol_semCode'])) {
        // get the default semester
        $semCode = $config->getConfig('Semester');
    }
    
    //set session variables
	$_SESSION[$_GET['module'].'regCol_regPaymentID']	= $regPaymentID;
	$_SESSION[$_GET['module'].'regCol_schYear']			= $schYear;
	$_SESSION[$_GET['module'].'regCol_semCode']			= $semCode;
	$_SESSION[$_GET['module'].'regCol_ORno']			= $ORno;
	$_SESSION[$_GET['module'].'regCol_type']			= $type;
	$_SESSION[$_GET['module'].'regCol_idno']			= $idno;
	$_SESSION[$_GET['module'].'regCol_lname']			= $lname;
	$_SESSION[$_GET['module'].'regCol_fname']			= $fname;
	$_SESSION[$_GET['module'].'regCol_mname']			= $mname;
	$_SESSION[$_GET['module'].'regCol_rstatus']			= $rstatus;
    if ($schYear) {
        if (count($conds[0])) {
    ...
    if ($semCode) {
        if (count($conds[0])) {
            $conds[0][' AND registration_payments.semCode'] = " = '$semCode' ";
        } else {
            $conds[0]['registration_payments.semCode'] = " = '$semCode' ";
        }
    }
...
//		$total_rec=0;
	...
	$main_url="index.php?module=Payments&action=listRegistrationPayments&regPaymentID=$regPaymentID&schYear=$schYear&semCode=$semCode&ORno=$ORno&idno=$idno&lname=$lname&fname=$fname&mname=$mname&type=$type&rstatus=$rstatus";
	...
	echo $sugar_smarty->fetch('modules/Payments/templates/listRegistrationPayments.tpl');
