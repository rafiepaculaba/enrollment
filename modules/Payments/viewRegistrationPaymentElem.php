...
$accessCode = $access->getAccessCode("View Elem Registration Payment");
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    $accessCode = $access->getAccessCode("Edit Elem Registration Payment");
    $sugar_smarty->assign('hasEdit', $access->check_access($current_user->id, $accessCode) );
    
    // to check if the user has an access in delete
    $accessCode = $access->getAccessCode("Cancel Elem Registration Payment");
    $sugar_smarty->assign('hasCancel', $access->check_access($current_user->id, $accessCode) );
	}

	echo $sugar_smarty->fetch('modules/Payments/templates/viewRegistrationPaymentElem.tpl');
	} else {
...
function cancelRegistrationPayment(regPaymentID, schYear, idno, amount, type)
{
    ...
    if (reply==true)
        redirect('index.php?module=Payments&action=cancelRegistrationPaymentElem&regPaymentID='+regPaymentID+'&schYear='+schYear+'&idno='+idno+'&amount='+amount+'&type='+type);
}
</script>