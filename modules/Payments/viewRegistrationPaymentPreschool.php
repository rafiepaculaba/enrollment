...
$accessCode = $access->getAccessCode("View Preschool Registration Payment");
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    $accessCode = $access->getAccessCode("Cancel Preschool Registration Payment");
    $sugar_smarty->assign('hasCancel', $access->check_access($current_user->id, $accessCode) );
	}

	echo $sugar_smarty->fetch('modules/Payments/templates/viewRegistrationPaymentPreschool.tpl');
	} else {
...
function cancelRegistrationPayment(regPaymentID, schYear, idno, amount, type)
{
    ...
    if (reply==true)
        redirect('index.php?module=Payments&action=cancelRegistrationPaymentPreschool&regPaymentID='+regPaymentID+'&schYear='+schYear+'&idno='+idno+'&amount='+amount+'&type='+type);
}
</script>