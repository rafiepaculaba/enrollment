...
echo $sugar_smarty->fetch('modules/Payments/templates/createRegistrationPayment.tpl');
...
<script>
addToValidate('frmRegistrationPayment','schYear', '', true, 'School Year');
addToValidate('frmRegistrationPayment','semCode', '', true, 'Semester');
addToValidate('frmRegistrationPayment','idno', '', true, 'ID No.');
addToValidate('frmRegistrationPayment','ORno', '', true, 'Official Receipt No.');
...
{
    get_data="schYear=" + $('schYear').value + "&semCode=" + $('semCode').value + "&idno=" + $('idno').value + "&action=validatepaymentcol";
...
{
    get_data="idno=" + $('idno').value + "&action=checkstudinformationcol";
...
function onCheckinformationHandle()
{
	if (xmlHttp.readyState==4) {
    	...
    	if (ret) {
    		//if id no exist
			if(ret == '[]' || ret == '0' ) {
				if ($('amount').value == '' || $('schYear').value == '' || $('semCode').value == '' || $('idno').value == '') {
				...
				} else if ($('amount').value == '' || $('schYear').value == '' || $('semCode').value == '' || $('idno').value == '')  {
					if (check_form('frmRegistrationPayment')) {
			    				$('frmRegistrationPayment').submit();	
			    	}
				} else {
