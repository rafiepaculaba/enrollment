...
if(file_exists('custom/modules/Notes/metadata/listviewdefs.php')){
    require_once('custom/modules/Notes/metadata/listviewdefs.php');  
}else{
    require_once('modules/Notes/metadata/listviewdefs.php');
}
...
if(!empty($_REQUEST['clear_query']) && $_REQUEST['clear_query'] == 'true')  
    $current_user->setPreference('ListViewDisplayColumns', array(), 0, 'Notes');

$savedDisplayColumns = $current_user->getPreference('ListViewDisplayColumns', 'Notes'); // get user defined display columns

$json = getJSONobj();

$seedNote = new Note(); // seed bean
$searchForm = new SearchForm('Notes', $seedNote); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['Notes'][$col])) 
            $displayColumns[$col] = $listViewDefs['Notes'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
    ...
    foreach($listViewDefs['Notes'] as $col => $params) {
...
if(!isset($_REQUEST['query'])){
    $storeQuery->loadQuery('Notes');
    $storeQuery->populateRequest();
}else{
    $storeQuery->saveFromGet('Notes');   
}
if(isset($_REQUEST['query']))
{
    // we have a query
    // first save columns 
    $current_user->setPreference('ListViewDisplayColumns', $displayColumns, 0, 'Notes'); 
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "Notes"); // builds the where clause from search field inputs
...
$lv->setup($seedNote, 'include/ListView/ListViewGeneric.tpl', $where, $params);
...
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('Notes')));
$str = "<script>
