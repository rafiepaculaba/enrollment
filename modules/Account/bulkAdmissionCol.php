...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME_ADMISSION'], $mod_strings['LBL_MODULE_TITLE_ADMISSION_COL'], false);
echo "\n</p>\n";
global $theme;
global $esConfig;
...
$accessCode = $access->getAccessCode("Generate Col Admission");
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    echo $sugar_smarty->fetch('modules/Account/templates/bulkAdmissionCol.tpl');
} else {
...
<script>
addToValidate('frmAdmission','schYear', '', true, 'School Year');
addToValidate('frmAdmission','semCode', '', true, 'Semester');
addToValidate('frmAdmission','term', '', true, 'Term');
addToValidate('frmAdmission','courseID', '', true, 'Course');
addToValidate('frmAdmission','yrLevel', '', true, 'Year');
</script>
...
function display()
{
    ...
    if (!msg) {
        if (confirm('Bulk printing. \nPress OK to continue then please wait while loading requested admission slips.')) {
            popUp('index.php?module=Account&action=printBulkAdmissionCol&schYear='+schYear+'&semCode='+semCode+'&term='+term+'&courseID='+courseID+'&yrLevel='+yrLevel+'&sugar_body_only=1');        
        }
    } else {
