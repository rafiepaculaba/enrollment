...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    echo $sugar_smarty->fetch('modules/Account/templates/bulkAssessmentElem.tpl');
} else {
...
addToValidate('frmAssessment','yrLevel', '', true, 'Grade');
</script>
...
function display()
{
    ...
    if (!msg) {
        if (confirm('Bulk printing. \nPress OK to continue then please wait while loading requested assessments.')) {
            popUp('index.php?module=Account&action=printBulkAssessmentElem&schYear='+schYear+'&term='+term+'&yrLevel='+yrLevel+'&sugar_body_only=1');        
        }
    } else {
