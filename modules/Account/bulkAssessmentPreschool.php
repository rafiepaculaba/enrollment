...
$accessCode = $access->getAccessCode("Generate PreSchool Assessment");
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    echo $sugar_smarty->fetch('modules/Account/templates/bulkAssessmentPreschool.tpl');
} else {
...
function display()
{
    ...
    if (!msg) {
        if (confirm('Bulk printing. \nPress OK to continue then please wait while loading requested assessments.')) {
            popUp('index.php?module=Account&action=printBulkAssessmentPreschool&schYear='+schYear+'&term='+term+'&yrLevel='+yrLevel+'&sugar_body_only=1');        
        }
    } else {
