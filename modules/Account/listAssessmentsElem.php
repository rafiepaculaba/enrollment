...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME_ASSESSMENT'], $mod_strings['LBL_MODULE_TITLE_ASSESSMENT_ELEM']."", false);
echo "\n</p>\n";
global $theme;
global $pageLimit;
...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($_GET['cmdFilter']) {
        ...
        $idno   = $_SESSION[$_GET['module'].'ElemAss_idno'];
        $lname  = $_SESSION[$_GET['module'].'ElemAss_lname'];
        $fname  = $_SESSION[$_GET['module'].'ElemAss_fname'];
        $mname  = $_SESSION[$_GET['module'].'ElemAss_mname'];
        $yrLevel  = $_SESSION[$_GET['module'].'ElemAss_yrLevel'];
        $schYear  = $_SESSION[$_GET['module'].'ElemAss_schYear'];
        $term     = $_SESSION[$_GET['module'].'ElemAss_term'];
    }
    
    
    if (!isset($_GET['schYear']) && !isset($_SESSION[$_GET['module'].'ElemAss_schYear'])) {
        // get the default schYear
        $schYear = $config->getConfig('School Year');
    }
    
    // set session variables
    $_SESSION[$_GET['module'].'ElemAss_idno']    = $idno;
    $_SESSION[$_GET['module'].'ElemAss_lname']   = $lname;
    $_SESSION[$_GET['module'].'ElemAss_fname']   = $fname;
    $_SESSION[$_GET['module'].'ElemAss_mname']   = $mname;
    $_SESSION[$_GET['module'].'ElemAss_yrLevel'] = $yrLevel;
    $_SESSION[$_GET['module'].'ElemAss_schYear'] = $schYear;
    $_SESSION[$_GET['module'].'ElemAss_term']    = $term;
...
//    $allAssessments = $assessment->retrieveAllAssessmentsAssociated($conds);
    ...
    $main_url="index.php?module=Account&action=listAssessmentsElem&idno=$idno&lname=$lname&fname=$fname&mname=$mname&term=$term&schYear=$schYear&yrLevel=$yrLevel";
    ...
    echo $sugar_smarty->fetch('modules/Account/templates/listAssessmentsElem.tpl');
} else {
