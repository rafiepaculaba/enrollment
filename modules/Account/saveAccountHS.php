...
require_once('modules/Account/AccountHSLog.php');

echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME'], $mod_strings['LBL_MODULE_TITLE_ACCT_HS'], false);
echo "\n</p>\n";
global $theme;
...
if ( isset($_POST['theForm']) ) {
	...
	if ($account->accID) {
	    ...
	    $account->idno        = $_POST['idno'];
        $account->schYear     = $_POST['schYear'];
        $account->tuitionFee  = $_POST['tuitionFee'];
        $account->labFee      = $_POST['labFee'];
        $account->regFee      = $_POST['regFee'];
        $account->miscFee     = $_POST['miscFee'];
        $account->addAdj      = $_POST['addAdj'];
        $account->lessAdj     = $_POST['lessAdj'];
        $account->oldBalance  = $_POST['oldBalance'];
        $account->totalFee    = $_POST['totalFee'];
        $account->payment     = $_POST['payment'];
        $account->balance     = $_POST['balance'];
...
setTimeout("redirect('index.php?module=Account&action=viewAccountHS&accID=<?php echo $account->accID; ?>')",3000);
</script>