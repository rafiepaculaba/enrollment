...
if ( isset($_POST['theForm']) && $schYear && $semCode && $term) {
    ...
    if ($student_enrollments) {
        foreach ($student_enrollments as $info) {
            ...
            if ($account->details) {
        	    ...
        	    foreach ($account->details as $data) {
        	        if (strtoupper($data['feeType'])=="ADD") {
                        ...
                        $labf += $data['amount'];
                    } else if (strtoupper($data['feeType'])=="TUITION" || strtoupper($data['feeType'])=="COMPUTER SUBJEC") {
                        $tfee += $data['amount'];
        	...
        	if ($term) {
        		...
        		$remaining_terms = $total_terms;
        	}
            
            // get the prev assessment of the student before this term
            $prev_assID = $assessment->isExist($student->idno, $accID, $schYear, $semCode, $term-1);
            ...
            //$paid = $regPayment->totalRegistrationPayment($student->idno, $schYear, $semCode);
            
            // get the amount paid during registration
            $orheader = new ORHeader();
            $payments = $orheader->getRegistrationPayment($student->idno, $schYear, $semCode);
            ...
            if (!$assessment->isExist($student->idno, $accID, $schYear, $semCode, $term)) {
                // new assessment
                $assessment->idno        = $student->idno;
                $assessment->accID       = $student->accID;
                $assessment->schYear     = $schYear;
                $assessment->semCode     = $semCode;
                $assessment->courseID    = $account->courseID;
