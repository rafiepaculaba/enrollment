...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME'], $mod_strings['LBL_MODULE_TITLE_ACCT_COL'], false);
echo "\n</p>\n";
global $theme;
        ...
        if ($account->details) {
            ...
            foreach ($account->details as $data) {
                ...
                if (strtoupper($data['feeType'])=="ADD") {
                    ...
                    if ( isset($_POST["labf_$accDetailID"]) ) {
                    	
                    	if ($data['amount']!=$_POST['labf_'.$data['accDetailID']]) {
                      		if ($changes_for_log) {
                      			$changes_for_log .= "<br>";
                      		}
                      		$changes_for_log .= $data['particular'].": ".$data['amount']." to ".$_POST['labf_'.$data['accDetailID']];
                      	}
                      	
                        $account_detail->amount      = $_POST['labf_'.$data['accDetailID']];
                        $account_detail->updateAccountDetail();
                    }
    	        } else {
...
setTimeout("redirect('index.php?module=Account&action=viewAccountCol&accID=<?php echo $account->accID; ?>')",3000);
</script>