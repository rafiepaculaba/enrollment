...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    echo $sugar_smarty->fetch('modules/Account/templates/bulkAssessmentCol.tpl');
} else {
...
function display()
{
    ...
    if (!msg) {
        if (confirm('Bulk printing. \nPress OK to continue then please wait while loading requested assessments.')) {
            popUp('index.php?module=Account&action=printBulkAssessmentCol&schYear='+schYear+'&semCode='+semCode+'&term='+term+'&courseID='+courseID+'&yrLevel='+yrLevel+'&sugar_body_only=1');        
        }
    } else {
