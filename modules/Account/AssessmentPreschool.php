...
/**
 *
 * Filename: AssessmentPreschool.php
 * Date: 	 June 27, 2008
 * 
...
class Assessment extends Database 
{
	 ...
	 */
	function retrieveAllAssessments($where='',$orderby='', $sorting='', $offset=0, $limit='')
	{
		// building an insert query
	    $this->tables = "assessments";
	    $this->conds  = $where;
	...
	function retrieveAllAssessmentsAssociated($where='',$orderby='assessments.schYear', $sorting='DESC', $offset=0, $limit='')
	{
		...
		if (count($where)) {
		     ...
		     $where[0]['students.idno']="=assessments.idno";
		}
	 ...
	 * @return bool 
	 */
	function isExist($idno, $accID, $schYear, $term)
	{
	    if ($idno && $accID && $schYear && $term) {
    	    // setting conditions
