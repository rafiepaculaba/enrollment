...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ( $accID ) {
    	...
    	echo $sugar_smarty->fetch('modules/Account/templates/editAccountCol.tpl');
	} else {
...
function recalculate()
{
    ...
    <?php
    if (is_array($lab_fees) && $lab_fees) {
        foreach ($lab_fees as $data) {
            echo "totalFee += parseFloat($('labf_".$data['accDetailID']."').value); \n";
        }
    }
    ?>
...
if ($lab_fees) {
    foreach ($lab_fees as $data) {
        echo "function lab_".$data['accDetailID']."()\n";
        echo "{";
        echo "    if (trim(\$('labf_".$data['accDetailID']."').value)==\"\") { \n";
        echo "        \$('labf_".$data['accDetailID']."').value=\"0.0\"; \n";
        echo "    } \n";
        echo "\n";    
        echo "    if (!isFloatNumber(\$('labf_".$data['accDetailID']."').value)) { \n";
        echo "        alert(\"Error: Invalid value in Tuition\"); \n";
        echo "        \$('labf_".$data['accDetailID']."').value=\"".$data['amount']."\"; \n";
        echo "    } else { \n";
        echo "        $('labf_".$data['accDetailID']."').value=parseFloat($('labf_".$data['accDetailID']."').value).toFixed(2); \n";
        echo "    } \n";
        echo "\n";    
        echo "    recalculate(); \n";
        echo "} \n\n";
