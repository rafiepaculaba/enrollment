...
$accessCode = $access->getAccessCode("View Col Assessment");
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ( $assID ) {
        ...
        if ( $assessment->schYear==$config->getConfig('School Year') && $assessment->semCode==$config->getConfig('Semester') ) {
	        $accessCode = $access->getAccessCode("Edit Col Assessment");
	        $sugar_smarty->assign('hasEdit', $access->check_access($current_user->id, $accessCode) );
        }
        
    	echo $sugar_smarty->fetch('modules/Account/templates/viewAssessmentCol.tpl');
	} else {
...
function popUp(URL) 
{
	day = new Date();
	id = day.getTime();
	eval("page" + id + " = window.open(URL, '" + id + "', 'toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,width=800,height=450,left = 0,top = 0');");
}
