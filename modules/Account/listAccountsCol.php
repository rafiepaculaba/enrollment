...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($_GET['cmdFilter']) {
        ...
        $idno       = $_SESSION[$_GET['module'].'ColAcc_idno'];
        $lname      = $_SESSION[$_GET['module'].'ColAcc_lname'];
        $fname      = $_SESSION[$_GET['module'].'ColAcc_fname'];
        $mname      = $_SESSION[$_GET['module'].'ColAcc_mname'];
        $gender     = $_SESSION[$_GET['module'].'ColAcc_gender'];
        $courseID   = $_SESSION[$_GET['module'].'ColAcc_courseID'];
        $yrLevel    = $_SESSION[$_GET['module'].'ColAcc_yrLevel'];
        $schYear    = $_SESSION[$_GET['module'].'ColAcc_schYear'];
        $semCode    = $_SESSION[$_GET['module'].'ColAcc_semCode'];
    }
    
    
    if (!isset($_GET['schYear']) && !isset($_SESSION[$_GET['module'].'ColAcc_schYear'])) {
        // get the default schYear
        $schYear = $config->getConfig('School Year');
    }
    
    if (!isset($_GET['semCode']) && !isset($_SESSION[$_GET['module'].'ColAcc_semCode'])) {
        // get the default semester
        $semCode = $config->getConfig('Semester');
    }
    
    // set session variables
    $_SESSION[$_GET['module'].'ColAcc_idno']        = $idno;
    $_SESSION[$_GET['module'].'ColAcc_lname']       = $lname;
    $_SESSION[$_GET['module'].'ColAcc_fname']       = $fname;
    $_SESSION[$_GET['module'].'ColAcc_mname']       = $mname;
    $_SESSION[$_GET['module'].'ColAcc_gender']      = $gender;
    $_SESSION[$_GET['module'].'ColAcc_courseID']    = $courseID;
    $_SESSION[$_GET['module'].'ColAcc_yrLevel']     = $yrLevel;
    $_SESSION[$_GET['module'].'ColAcc_schYear']     = $schYear;
    $_SESSION[$_GET['module'].'ColAcc_semCode']     = $semCode;
    ...
    if ($courseID) {
        if (count($conds[0])) {
            $conds[0][' AND students.courseID  '] = " = '$courseID' ";
        } else {
            $conds[0]['students.courseID'] = " = '$courseID' ";
        }
    }
    ...
    if ($semCode) {
        if (count($conds[0])) {
            $conds[0][' AND accounts.semCode'] = " = '$semCode' ";
        } else {
            $conds[0]['accounts.semCode'] = " = '$semCode' ";
        }
    }
...
//    $allAccounts = $account->retrieveAllAccountsAssociation($conds);
    ...
    $list        = $account->retrieveAllAccountsAssociation($conds,"students.lname","ASC",$offset, $limit);
	$total_rec = $allAccounts;
    	
    $main_url="index.php?module=Account&action=listAccountsCol&idno=$idno&lname=$lname&fname=$fname&mname=$mname&gender=$gender&courseID=$courseID&yrLevel=$yrLevel&schYear=$schYear&semCode=$semCode";
    ...
    echo $sugar_smarty->fetch('modules/Account/templates/listAccountsCol.tpl');
} else {
