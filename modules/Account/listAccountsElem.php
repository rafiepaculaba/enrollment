...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($_GET['cmdFilter']) {
        ...
        $idno   = $_SESSION[$_GET['module'].'ElemAcc_idno'];
        $lname  = $_SESSION[$_GET['module'].'ElemAcc_lname'];
        $fname  = $_SESSION[$_GET['module'].'ElemAcc_fname'];
        $mname  = $_SESSION[$_GET['module'].'ElemAcc_mname'];
        $yrLevel  = $_SESSION[$_GET['module'].'ElemAcc_yrLevel'];
        $schYear  = $_SESSION[$_GET['module'].'ElemAcc_schYear'];
    }
    
    if (!isset($_GET['schYear']) && !isset($_SESSION[$_GET['module'].'ElemAcc_schYear'])) {
        // get the default schYear
        $schYear = $config->getConfig('School Year');
    }
    
    // set session variables
    $_SESSION[$_GET['module'].'ElemAcc_idno']    = $idno;
    $_SESSION[$_GET['module'].'ElemAcc_lname']   = $lname;
    $_SESSION[$_GET['module'].'ElemAcc_fname']   = $fname;
    $_SESSION[$_GET['module'].'ElemAcc_mname']   = $mname;
    $_SESSION[$_GET['module'].'ElemAcc_yrLevel'] = $yrLevel;
    $_SESSION[$_GET['module'].'ElemAcc_schYear'] = $schYear;
...
//    $allAccounts = $account->retrieveAllAccountsAssociation($conds);
    ...
    $main_url="index.php?module=Account&action=listAccountsElem&idno=$idno&lname=$lname&fname=$fname&mname=$mname&yrLevel=$yrLevel&schYear=$schYear";
    ...
    foreach ($elementary_yrs as $key=>$val) {
        if ($key==$yrLevel)
    ...
    echo $sugar_smarty->fetch('modules/Account/templates/listAccountsElem.tpl');
} else {
