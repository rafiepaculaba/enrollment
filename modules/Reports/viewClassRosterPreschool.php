...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ($user->groupID==11) {
		// the current user is a Preschool Teacher
		$conds[0]['id'] = "='".$user->id."' "; // assuming 6-default groupID of Preschool Teacher
		...
		// get all professors
		$user = new User2();
		$conds[0]['groupID'] = "='11' "; // assuming 11-default groupID of Preschool Teacher
		$profList = $user->retrieveAllUsers($conds);
	    $sugar_smarty->assign('PROFLIST', $profList);
    ...
    echo $sugar_smarty->fetch('modules/Reports/templates/viewClassRosterPreschool.tpl');
} else {
...
<script>
addToValidate('frmClassRosterPreschool','schYear', '', true, 'School Year');
addToValidate('frmClassRosterPreschool','profID', '', true, 'Teacher');
addToValidate('frmClassRosterPreschool','schedID', '', true, 'Sched Code');
</script>
...
function setPrinter()
{
	...
	thePrinter = '<a href="#" onclick="popUp(\'index.php?module=Reports&action=classRosterPreschool&schedID=' + schedID + '&sugar_body_only=1\');" id="print_link"><img src="themes/Sugar/images/print.gif" alt="Export" align="absmiddle" border="0" height="9" width="11">&nbsp;Print</a>';
