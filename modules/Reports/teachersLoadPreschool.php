...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME_CUR'], $mod_strings['LBL_MODULE_TITLE_TL_PRESCHOOL'], false);
echo "\n</p>\n";
global $theme;
global $esConfig;
...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ($_POST['cmdGo']) {
		...
		$sugar_smarty->assign('RESULT', $reportClass->teachersLoadPreschool($record));
	...
	if ($user->groupID==11) {
		...
		$where[0]['id'] = "='".$user->id."' "; // assuming 8-default groupID of Preschool Teacher
    ...
    echo $sugar_smarty->fetch('modules/Reports/templates/teachersLoadPreschool.tpl');	
} else {
...
<script>
addToValidate('frmTeachersLoadPreschool','schYear', '', true, 'School Year');
addToValidate('frmTeachersLoadPreschool','profID', '', true, 'Teacher');
</script>
