...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME_CUR'], $mod_strings['LBL_MODULE_TITLE_HS'], false);
echo "\n</p>\n";
global $theme;
global $esConfig;
...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ($user->groupID==7) {
			// the current user is a high school Teacher
	    ...
	    $sugar_smarty->assign('isHSGroup', 1);
		
	} else {
		// get all professors
		$user = new User2();
		$conds[0]['groupID'] = "='7' "; // assuming 7-default groupID of High School Teacher
		$profList = $user->retrieveAllUsers($conds);
	    $sugar_smarty->assign('PROFLIST', $profList);
	    $sugar_smarty->assign('isHSGroup', 0);
	}
    ...
    echo $sugar_smarty->fetch('modules/Reports/templates/searchClassRosterHS.tpl');
} else {
...
addToValidate('frmClassRoster','profID', '', true, 'Teacher');
...
function checkDuplicate()
{
    if ( trim($('profID').value) != "" && trim($('schedID').value) != "" && trim($('schYear').value) != "" ) {
        get_data="profID=" + $('profID').value + "&schedID=" + $('schedID').value + "&schYear=" + $('schYear').value + "&action=checkduplicatehs";
...
function getSchedules()
{
	...
	$('students').innerHTML = '	<table class="listView" border="0" cellpadding="0" cellspacing="0" width="100%"><tbody>	<tr height="20">	    <td scope="col" class="listViewThS1" width="5%" nowrap>&nbsp;</td>	    <td scope="col" class="listViewThS1" width="20%" nowrap>ID No.</td>		<td scope="col" class="listViewThS1" width="65%" nowrap>Student Name</td>		<td scope="col" class="listViewThS1" width="10%" nowrap>Year</td>	</tr></tbody></table>';
	
    get_data="schYear=" + $('schYear').value + "&profID=" + $('profID').value + "&action=getscheduleshs";
...
function getSubject()
{
   if (trim($('schedID').value)!="") {
        get_data="schedID=" + $('schedID').value + "&action=getsubjecths";
...
{
    if ( trim($('schedID').value) != "" ) {
        get_data="schedID=" + $('schedID').value + "&action=getclassrosterhs";
...
function setPrinter()
{
	...
	thePrinter = '<a href="#" onclick="popUp(\'index.php?module=Reports&action=classRosterHS&schedID=' + schedID + '&sugar_body_only=1\');" id="print_link"><img src="themes/Sugar/images/print.gif" alt="Export" align="absmiddle" border="0" height="9" width="11">&nbsp;Print</a>';
