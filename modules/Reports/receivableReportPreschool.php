...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME_CUR'], $mod_strings['LBL_MODULE_TITLE_RR_PRESCHOOL'], false);
echo "\n</p>\n";
global $theme;
global $esConfig;
...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($_POST['theForm']) {
        ...
        while ($ctr<=5) {
            ...
            if ($ctr<5) {
                ...
                $query="SELECT sum( ttlDue-amtPaid ) AS total_amount FROM assessments  WHERE term='$term' AND schYear = '$schYear' AND yrLevel = 'S' AND rstatus>0 AND ttlDue>amtPaid";
            }
        ...
        $sugar_smarty->assign('RESULT', $reportClass->receivableReportPreschool($result, $total));
    ...
    echo $sugar_smarty->fetch('modules/Reports/templates/receivableReportPreschool.tpl');	
} else {
