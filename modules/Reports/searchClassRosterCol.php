...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME_CUR'], $mod_strings['LBL_MODULE_TITLE_COL'], false);
echo "\n</p>\n";
global $theme;
global $esConfig;
...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    echo $sugar_smarty->fetch('modules/Reports/templates/searchClassRosterCol.tpl');	
} else {
	...
	if (xmlHttp.readyState==4) {
    	...
    	if (ret) {
			...
			for(c = 0; c < myData.length; c++){
		    	var y=document.cRemarks  	 reateElement('option');
