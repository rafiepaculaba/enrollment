...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME_CUR'], $mod_strings['LBL_MODULE_TITLE_TL_HS'], false);
echo "\n</p>\n";
global $theme;
global $esConfig;
...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ($_POST['cmdGo']) {
		...
		$sugar_smarty->assign('RESULT', $reportClass->teachersLoadHS($record));
	...
	if ($user->groupID==7) {
		// the current user is a HS teacher
		$where[0]['id'] = "='".$user->id."' "; // assuming 7-default groupID of HS Teacher
    ...
    echo $sugar_smarty->fetch('modules/Reports/templates/teachersLoadHS.tpl');	
} else {
...
<script>
addToValidate('frmTeachersLoadHS','schYear', '', true, 'School Year');
addToValidate('frmTeachersLoadHS','profID', '', true, 'Teacher');
</script>
