...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ($cre->creID) {
	    ...
	    if ( !empty($cre->creID) ) {
		    ...
		    $conds[0]['tor.creID'] = "= '$cre->creID'";
			
			$result = $tor->retrieveAllTORs($conds);
			$torD = new TOR($result[0][torID]);
			$torD->deleteTOR();
			
	        if ($cre->deleteCreditedSubject()) {
...
setTimeout("redirect('index.php?module=Credited&action=listCreditedSubjects')", 3000);
</script>