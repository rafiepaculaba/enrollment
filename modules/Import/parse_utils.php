...
//	date_modified is not importable.. because the value is set during save.
function get_importable_fields(&$bean, &$importable_fields, &$labels) {
	...
	foreach ($my_fielddefs as $key=>$value_array) {		
		if ((array_key_exists('Importable',$value_array) && $value_array['Importable'] == false )
			or ((array_key_exists('type',$value_array) && $value_array['type'] == 'link' ) or $key == 'team_name' or $key == 'date_modified')
			) {
				//do not allow import.
			} else {
