...
$fromsemCode= $_POST['fromsemCode'];
$tosemCode  = $_POST['tosemCode'];
...
if ($fromsemCode) {
    $conds[0]['semCode'] = " = '$fromsemCode' AND ";
}
if ($fromschYear) {
...
if (!count($list)) {
	...
	while ($ctr <= count($list)) {
		...
		$sched->semCode		=	$tosemCode;
		$sched->curID		=	$list[$ctr]['curID'];
		$sched->schedCode	=	$list[$ctr]['schedCode'];
		$sched->courseID	=	$list[$ctr]['courseID'];
	    ...
	    if(($sched->isExist($toschYear, $tosemCode, $list[$ctr]['schedCode'])) == -1) {
		// new schedule record
