...
$accessCode = $access->getAccessCode("View Preschool Subject");
...
//	$sugar_smarty->assign('yrLevel', $subj->yrLevel);
    ...
    $accessCode = $access->getAccessCode("Delete Preschool Subject");
    $sugar_smarty->assign('hasDelete', $access->check_access($current_user->id, $accessCode) );

	
	echo $sugar_smarty->fetch('modules/Subjects/templates/viewSubjectPreschool.tpl');
} else {
...
function deleteSubject(subjID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Subjects&action=deleteSubjectPreschool&subjID='+subjID);
}
</script>