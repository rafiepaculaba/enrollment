...
if ($col_enabled) {
    $ac1=$access->getAccessCode("List Col Subject");
}
if ($hs_enabled) {
    $ac2=$access->getAccessCode("List HS Subject");
}
if ($elem_enabled) {
    $ac3=$access->getAccessCode("List Elem Subject");
}
if ($pre_enabled) {
    $ac4=$access->getAccessCode("List Preschool Subject");
}

if ($access->check_access($current_user->id,$ac1)) {
include('modules/Subjects/listSubjects.php');
} else if ($access->check_access($current_user->id,$ac2)) {
include('modules/Subjects/listSubjectsHS.php');
} else if ($access->check_access($current_user->id,$ac3)) {
include('modules/Subjects/listSubjectsElem.php');
} else if ($access->check_access($current_user->id,$ac4)) {
include('modules/Subjects/listSubjectsPreschool.php');
}
?>
