...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    echo $sugar_smarty->fetch('modules/Students/templates/createStudentElem.tpl');
} else {
...
function checkDuplicate()
{
    if (check_form('frmStudent')) {
        get_data="idno=" + $('idno').value + "&action=checkDuplicate_elem";
...
function checkDuplicateHandle() 
{
	if (xmlHttp.readyState==4) {
    	...
    	if (ret=='') {
    		redirect('index.php?module=Students&action=createStudentElem&regID=<?php echo $_GET['regID']; ?>'); // redirect to entry page
    	} else {
	    	if (ret==-1) {
	    		// ID No. duplicate
                //$('frmStudent').submit();
