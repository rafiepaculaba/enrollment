...
$target_path = "uploads/col_students/";
...
if ( isset($_POST['theForm']) ) {
	...
	if ($stud->recID) {
        ...
        $stud->primary_edu    = trim($_POST['primary_edu']);
        $stud->interm_edu     = trim($_POST['interm_edu']);
        $stud->hs_edu     	  = trim($_POST['hs_edu']);
        $stud->primary_schYear= trim($_POST['primary_schYear']);
        $stud->interm_shcYear = trim($_POST['interm_shcYear']);
        $stud->hs_schYear     = trim($_POST['hs_schYear']);
        
        if ($_POST['docs']) {
        	$ctr = 0;
        	foreach ($_POST['docs'] as $value) {
        		if ($ctr)
        			$stud->entryDocs .= ", ".$value;
        		else 
        			$stud->entryDocs = $value;
...
//        $stud->entryDocs      = trim($_POST['entryDocs']);
        ...
        $stud->courseID      = trim($_POST['courseID']);
        $stud->curID         = trim($_POST['curID']);
...
setTimeout("redirect('index.php?module=Students&action=viewStudent&idno=<?php echo $stud->idno; ?>')",3000);
</script>