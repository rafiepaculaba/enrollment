...
$accessCode = $access->getAccessCode("View HS Student");
    ...
    if (!$idno) {
        ...
        if ( $stud->isExist($idno) ) {
            ...
            foreach ($allowed_filetypes as $value) {
                if (is_file("uploads/hs_students/".$stud->idno.$value)) {
                    $image = "uploads/hs_students/".$stud->idno.$value;
                    break;
                }	
            }
            ...
            echo $sugar_smarty->fetch('modules/Students/templates/viewStudentHS.tpl');
...
function deleteStudent(idno)
{
    ...
    if (reply==true)
        redirect('index.php?module=Students&action=deleteStudentHS&idno='+idno);
}
</script>