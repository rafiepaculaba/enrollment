...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    // list of registrant
    $conds[0]['rstatus'] = "= 1 ";
    $list = $reg->retrieveAllRegistrations($conds);
    ...
    echo $sugar_smarty->fetch('modules/Students/templates/createStudentPreschool.tpl');
} else {
...
function checkDuplicate()
{
    if (check_form('frmStudent')) {
        get_data="idno=" + $('idno').value + "&action=checkDuplicate_preschool";
...
function checkDuplicateHandle() 
{
	if (xmlHttp.readyState==4) {
    	...
    	if (ret=='') {
    		redirect('index.php?module=Students&action=createStudentPreschool&regID=<?php echo $_GET['regID']; ?>'); // redirect to entry page
    	} else {
	    	if (ret==-1) {
	    		// ID No. duplicate
                //$('frmStudent').submit();
