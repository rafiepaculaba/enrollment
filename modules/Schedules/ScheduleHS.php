...
class Schedule extends Database 
{
	...
	function Schedule($id='') 
	{
	    ...
	    parent::Database(2); // college level
...
//                    $this->profID 			= $data[0]['profID'];
	...
	/**
	 * retrieveAllSchedulesSubjectAssociated() method to retrieve all/selected schedule records
	 *
	 * @param array $where
	 * @param string $orderby
	 * @param string $sorting
	...
	/**
	 * isExist() - function to check the schedCode is already exist
	 *
	 * @param string $lname
	 * @param string $fname
	 * @param string $mname
	    ...
	    if ($schYear && $schedCode) {
    		...
    		if ( empty($this->errs) ) {
        	    try {
            		...
            		if ($data[0]) {
                        // return true - the schedule is already exist            		
                		return 1;
                	} else {
                	    // return false - the schedule does not exist
                	    return -1;
                	}   		
        	    } catch(PDOException $e) {
        	        echo $e;
