...
$accessCode = $access->getAccessCode("View Elem Schedule");
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    $accessCode = $access->getAccessCode("Delete Elem Schedule");
    $sugar_smarty->assign('hasDelete', $access->check_access($current_user->id, $accessCode) );
	
	
	echo $sugar_smarty->fetch('modules/Schedules/templates/viewScheduleElem.tpl');
} else {
...
function deleteSchedule(schedID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Schedules&action=deleteScheduleElem&schedID='+schedID);
}
</script>