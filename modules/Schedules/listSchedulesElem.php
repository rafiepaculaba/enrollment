...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ($_GET['cmdFilter']) {	
		...
		$subjID     	= $_SESSION[$_GET['module'].'Elem_subjID'];			
		$noEnrolled     = $_SESSION[$_GET['module'].'Elem_noEnrolled'];			
		$profID     	= $_SESSION[$_GET['module'].'Elem_profID'];			
		$room  		   	= $_SESSION[$_GET['module'].'Elem_room'];			
	...
	if ($user->groupID==8) {
	    ...
	    $sugar_smarty->assign('isElemGroup', 1);
	    $profID=$user->id;
	} else {
		//User list
		$where[0]['groupID']="=8";
		$sugar_smarty->assign('isElemGroup', 0);
	}
	...
	$_SESSION[$_GET['module'].'Elem_subjID']		= $subjID;
	$_SESSION[$_GET['module'].'Elem_noEnrolled']	= $noEnrolled;
	$_SESSION[$_GET['module'].'Elem_profID']		= $profID;
	$_SESSION[$_GET['module'].'Elem_room']			= $room;
...
//		$total_rec=0;
	...
	$main_url="index.php?module=Schedules&action=listSchedulesElem&schYear=$schYear&yrLevel=$yrLevel&schedCode=$schedCode&subjID=$subjID&noEnrolled=$noEnrolled&profID=$profID&rstatus=$rstatus";
    ...
    $print_link = "index.php?module=Schedules&action=printSchedulesElem&schYear=$schYear&droom=$droom&dnoEnrolled=$dnoEnrolled&dmaxCapacity=$dmaxCapacity&dstartTime=$dstartTime&dendTime=$dendTime&dremarks=$dremarks&ddays=$ddays&sugar_body_only=1";
	...
	echo $sugar_smarty->fetch('modules/Schedules/templates/listSchedulesElem.tpl');
} else {
