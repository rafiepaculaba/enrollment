...
<table width="100%" border="0">
  <tr>
    ...
    <input class="button" name="cmdCancel" type="button" id="cmdCancel" value=" Cancel " onclick="redirect('index.php?module=Schedules&action=listBlockSectionsCol')" />
    </td>
...
<tr><td>
    ...
    <tr>
        ...
        <td class="dataField" colspan="3"><slot><input type="text" name="secName" id="secName" maxlength="20" onkeypress="return keyRestrict(event, 5);"/> </slot>  </td>
    </tr>
    <tr>
        <td class="dataLabel"><slot>Course <span class="required">*</span></slot> </td>
        <td class="dataField">
        <slot> 
        <select name="courseID" id="courseID" onchange="getSchedules(0);">
        <option value="">--------------------------------</option>
...
<table class="tabForm" border="0" cellpadding="0" cellspacing="0" width="100%">
    ...
    <tr>
        ...
        <option value="{$schedList[i].schedID}">{$schedList[i].schedCode} - {$schedList[i].subjCode}</option>
        {/section}
        </select>
		</slot>
