...
if ($access->check_access($current_user->id, $accessCode)) {
    ...
    $semesters='<select name="semCode" id="semCode" disabled onchange="getSchedules();">'."\n";
    $semesters.='<option value="">------------</option>'."\n";
    if ($esConfig['semesters']) {
        foreach ($esConfig['semesters'] as $key=>$value) {
            if ($blocksection->semCode==$key) {
	...
	$schYear='<select name="schYear" id="schYear" disabled onchange="getSchedules();">'."\n";
    ...
    echo $sugar_smarty->fetch('modules/Schedules/templates/editBlockSectionCol.tpl');
...
{
//    if ($('schYear').value && $('yrLevel').value && $('semCode').value) {
	...
	if (xmlHttp.readyState==4) {
    	...
    	if (ret) {
			...
			for(c = 0; c < myData.length; c++){
				...
				y.text=myData[c].schedCode + " - " + myData[c].subjCode  + " " + myData[c].descTitle;
