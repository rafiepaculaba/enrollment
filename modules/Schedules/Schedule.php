...
class Schedule extends Database 
{
	...
	function createSchedule() 
	{
        ...
        $info['noReserved']     = $this->noReserved;
	...
	function retrieveSchedule($lock='0')
	{
		...
		$flds['curriculums.curName'] = "";
		
		$where[0][' AND schedules.subjID'] = "= subjects.subjID ";
	    $where[0][' AND schedules.courseID'] = "= courses.courseID ";
	    $where[0][' AND schedules.curID'] = "= curriculums.curID ";
...
//	    $this->conds  = $conds;
		...
		if ( empty($this->errs) ) {
    	    try {
        		...
        		if ($data[0]) {
                    ...
                    //$cur = new Curriculum($data[0]['curID']);                 
                    ...
                    $this->noReserved     	= $data[0]['noReserved'];
	...
	function updateSchedule()
	{
        ...
        $info['labFee']     	= $this->labFee;
        $info['remarks']        = $this->remarks;
        if (($this->noEnrolled+$this->noReserved)>=$this->maxCapacity) {
	...
	/**
	 * updateScheduleLab() method to update the schedule record with the active schedID
	 *
	 * @return bool - return true if successful otherwise false
	 */
	function updateScheduleLab()
	{
	    // setting values to fields
        $info['labFee']     	= $this->labFee;
...
//    		return $data;
	 ...
	 * @return array recordset
	 */
	function retrieveAllSchedulesSubjectCourseAssociated($where='',$orderby='', $sorting='ASC', $offset=0, $limit='')
	{
		// building an insert query
		$tables[]="schedules";
		$tables[]="subjects";
		...
		$multiOrder['schedules.semCode']  = "DESC";
	...
	/**
	 * countAllSchedulesAssociated() method to retrieve all/selected schedule records
	 *
	 * @param array $where
	 * @param int $offset
	 * @param int $limit
	 ...
	 * @return bool 
	 */
	function isExist($schYear, $semCode, $schedCode)
	{
	    if ($schYear && $semCode && $schedCode) {
    	    // setting conditions
     ...
     * @return integer [1|0] 
     */
    function isConflict($target, $scheds, $schYear, $semCode, $schedCode)
    {
        // convert to array
        $target['days']=array();
        if ($target['onMon']) {
        ...
        for($i=0;$i<count($scheds);$i++) {
            ...
            if ($scheds[$i]['schYear'] == $schYear && $scheds[$i]['semCode'] == $semCode && $scheds[$i]['schedCode'] != $schedCode) {
