...
$accessCode = $access->getAccessCode("Edit Preschool Schedule");
...
//	$sugar_smarty->assign('maxCapacity', $sched->maxCapacity );
	...
	echo $sugar_smarty->fetch('modules/Schedules/templates/editSchedulePreschool.tpl');
...
<script>
addToValidate('frmSchedulePreschool','schYear', '', true, 'School Year');
addToValidate('frmSchedulePreschool','yrLevel', '', true, 'Grade Level');
addToValidate('frmSchedulePreschool','subjID', '', true, 'Subject');
addToValidate('frmSchedulePreschool','schedCode', '', true, 'Schedule');
//addToValidate('frmSchedulePreschool','maxCapacity', '', true, 'Max Capacity');
addToValidate('frmSchedulePreschool','room', '', true, 'Room');
</script>
...
{
	if (check_form('frmSchedulePreschool')) {
...
{ 
	...
	if($('prevroom').value != $('room').value || $('prevschYear').value != $('schYear').value || Number($('prevshh').value) != $('shh').value || Number($('prevsmm').value) != $('smm').value || $('prevsamp').value != $('samp').value || Number($('prevehh').value) != $('ehh').value || Number($('prevemm').value) != $('emm').value || $('preveamp').value != $('eamp').value || $('prevonMon').value != onMon || $('prevonTue').value != onTue || $('prevonWed').value != onWed || $('prevonThu').value != onThu || $('prevonFri').value != onFri || $('prevonSat').value != onSat || $('prevonSun').value != onSun) {
		get_data="schYear=" + $('schYear').value + "&schedCode=" + $('schedCode').value + "&startTime=" + startTime + "&endTime=" + endTime + "&onMon=" + onMon + "&onTue=" + onTue + "&onWed=" + onWed + "&onThu=" + onThu + "&onFri=" + onFri + "&onSat=" + onSat + "&onSun=" + onSun + "&room=" + $('room').value + "&action=checkconflictpreschool";
...
function checkDuplicate()
{
    ...
    get_data="schYear=" + $('schYear').value + "&schedCode=" + $('schedCode').value + "&action=checkduplicatepreschool";
    ajaxQuery("modules/Schedules/scheduleHandler.php",'GET',get_data,"","onCheckDuplicateHandle");
	} else {
		
		$('frmSchedulePreschool').submit();
	}
}
...
function onCheckDuplicateHandle()
{
	if (xmlHttp.readyState==4) {
    	...
    	var ret = xmlHttp.responseText;
    	if (ret=='') {
    		redirect('index.php?module=Schedules&action=createSchedulePreschool'); // redirect to entry page
    	} else {
	    	if (ret==-1) {
	    		// ID No. duplicate
           		// okay alright..
