...
$accessCode = $access->getAccessCode("View HS School Fee");
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    $accessCode = $access->getAccessCode("Delete HS School Fee");
    $sugar_smarty->assign('hasDelete', $access->check_access($current_user->id, $accessCode) );

	
	echo $sugar_smarty->fetch('modules/Administration/templates/viewSchoolFeeHS.tpl');
...
function deleteSchoolFee(feeID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Administration&action=deleteSchoolFeeHS&feeID='+feeID);
}
</script>