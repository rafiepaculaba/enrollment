...
$accessCode = $access->getAccessCode("View Col Configuration");
if ($access->check_access($current_user->id,$accessCode)) {
    $typeID = $_GET['typeID'];
    $chartaccounttype = new ChartAccountType($typeID);
    $sugar_smarty->assign('parentType', $chartaccounttype->parent );
    $sugar_smarty->assign('account_name', $chartaccounttype->name );
    $sugar_smarty->assign('account_name', $chartaccounttype->id );
    
    // to check if the user has an access in edit
    $accessCode = $access->getAccessCode("Edit Col Configuration");
    ...
    echo $sugar_smarty->fetch('modules/Administration/templates/viewChartOfAccountType.tpl');
} else {
...
function deleteConfig(configID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Config&action=deleteConfigCol&configID='+configID);
}
</script>