...
$accessCode = $access->getAccessCode("View Elem Grade");
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($gs->rstatus<2) {
        ...
        $accessCode = $access->getAccessCode("Approve Elem Grade");
    ...
    if ($gs->rstatus==2) {
        ...
        if ($u->groupID == 9 || $current_user->is_admin) {
            // to check if the user has an access in edit
            $accessCode = $access->getAccessCode("Edit Elem Grade");
            $sugar_smarty->assign('hasEdit', $access->check_access($current_user->id, $accessCode) );
        }
        ...
        $accessCode = $access->getAccessCode("Post Elem Grade");
        $sugar_smarty->assign('hasPost', $access->check_access($current_user->id, $accessCode) );
    }
    
    echo $sugar_smarty->fetch('modules/Grades/templates/viewGradeSheetElem.tpl');
} else {
...
function approveGradeSheet(gsID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Grades&action=approveGradesheetElem&gsID='+gsID);
}
...
function deleteGradeSheet(gsID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Grades&action=deleteGradesheetElem&gsID='+gsID);
}
...
function postGradeSheet(gsID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Grades&action=postGradesheetElem&gsID='+gsID);
}
