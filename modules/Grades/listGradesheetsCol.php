...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($_GET['cmdFilter']) {
        ...
        $gsID      = $_SESSION[$_GET['module'].'Col_gsID'];
	...
	if ($u->groupID==6) {
	    ...
	    $where[0]['groupID']="=6";  // assuming the groupID 6 is college insctructor
	    $sugar_smarty->assign('isInstructor', 0);
	}

	$prof_list = $u->retrieveAllUsers($where,'');
	$sugar_smarty->assign('profList', $prof_list);
    
    
    // set session variables
    $_SESSION[$_GET['module'].'Col_gsID']       = $gsID;
    ...
    if ($semCode) {
        if (count($conds[0])) {
            $conds[0][' AND grade_sheets.semCode'] = " = '$semCode' ";
        } else {
            $conds[0]['grade_sheets.semCode'] = " = '$semCode' ";
        }
    }
    ...
    $allGS = $gs->retrieveAllGradeSheetsAssociated($conds,"subjects.subjCode","ASC",'','',$dept);
    $list  = $gs->retrieveAllGradeSheetsAssociated($conds,"subjects.subjCode","ASC",$offset, $limit,$dept);
    ...
    $main_url="index.php?module=Grades&action=listGradesheetsCol&gsID=$gsID&schedCode=$schedCode&subjCode=$subjCode&profID=$profID&schYear=$schYear&semCode=$semCode&rstatus=$rstatus";
    ...
    echo $sugar_smarty->fetch('modules/Grades/templates/listGradesheetsCol.tpl');
} else {
