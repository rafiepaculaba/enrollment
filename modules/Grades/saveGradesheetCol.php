    ...
    if (!$gsID) {
        if (!$gs->isExist($schYear, $semCode, $schedID, $profID)) {
            ...
            if ($gs->createGradeSheet()) {
                $lastID = $gs->getLastID($schYear, $semCode, $schedID, $profID);
                $error = 0;
            } else {
                $error = 1;
            }
    ...
    if (!$error) {
    	...
    	if ($records) {
    	    ...
    	    foreach ($records as $stud) {
    	        ...
    	        // update prelim grades
    	        if (is_numeric($_POST['stud_pre'.$stud['recID']])) {
    	           $enrollmentDetail->pregrade=number_format($_POST['stud_pre'.$stud['recID']],1); 
    	        } else {
    	           $enrollmentDetail->pregrade=$_POST['stud_pre'.$stud['recID']];
    	        }
    	        // update mgrades
    	        if (is_numeric($_POST['stud_mid'.$stud['recID']])) {
    	           $enrollmentDetail->mgrade=number_format($_POST['stud_mid'.$stud['recID']],1); 
    	        } else {
    	           $enrollmentDetail->mgrade=$_POST['stud_mid'.$stud['recID']];
    	        }

    	        // update prefigrades
    	        if (is_numeric($_POST['stud_prefi'.$stud['recID']])) {
    	           $enrollmentDetail->prefigrade=number_format($_POST['stud_prefi'.$stud['recID']],1); 
    	        } else {
    	           $enrollmentDetail->prefigrade=$_POST['stud_prefi'.$stud['recID']];
    	        }
    	        // update fgrades
    	        if (is_numeric($_POST['stud'.$stud['recID']])) {
    	           $enrollmentDetail->fgrade=number_format($_POST['stud'.$stud['recID']],1); 
    	        } else {
...
setTimeout("redirect('index.php?module=Grades&action=viewGradeSheetCol&gsID=<?php echo $lastID; ?>')",3000);
</script>