...
$accessCode = $access->getAccessCode("View Col Grade");
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($gs->rstatus<2) {
        ...
        $accessCode = $access->getAccessCode("Delete Col Grade");
        ...
        $accessCode = $access->getAccessCode("Approve Col Grade");
    ...
    if ($gs->rstatus==2) {
        ...
        if ($u->groupID == 5 || $current_user->is_admin) {
            // to check if the user has an access in edit
            $accessCode = $access->getAccessCode("Edit Col Grade");
            $sugar_smarty->assign('hasEdit', $access->check_access($current_user->id, $accessCode) );
        }
    ...
    echo $sugar_smarty->fetch('modules/Grades/templates/viewGradeSheetCol.tpl');
} else {
...
function approveGradeSheet(gsID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Grades&action=approveGradesheetCol&gsID='+gsID);
}
...
function deleteGradeSheet(gsID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Grades&action=deleteGradesheetCol&gsID='+gsID);
}

function postGradeSheet(gsID)
{
    reply=confirm("Do you want to continue posting grades to the TOR?");
    
    if (reply==true)
        redirect('index.php?module=Grades&action=postGradesheetCol&gsID='+gsID);
}
