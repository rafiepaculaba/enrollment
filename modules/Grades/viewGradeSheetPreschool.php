...
$accessCode = $access->getAccessCode("View Preschool Grade Sheet");
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if ($gs->rstatus<2) {
        ...
        $accessCode = $access->getAccessCode("Approve Preschool Grade");
    ...
    if ($gs->rstatus==2) {
        ...
        if ($u->groupID == 11 || $current_user->is_admin) {
            // to check if the user has an access in edit
            $accessCode = $access->getAccessCode("Edit Preschool Grade");
            $sugar_smarty->assign('hasEdit', $access->check_access($current_user->id, $accessCode) );
        }
        ...
        $accessCode = $access->getAccessCode("Post Preschool Grade");
        $sugar_smarty->assign('hasPost', $access->check_access($current_user->id, $accessCode) );
    }
    
    echo $sugar_smarty->fetch('modules/Grades/templates/viewGradeSheetPreschool.tpl');
} else {
...
function approveGradeSheet(gsID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Grades&action=approveGradesheetPreschool&gsID='+gsID);
}
...
function deleteGradeSheet(gsID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Grades&action=deleteGradesheetPreschool&gsID='+gsID);
}
...
function postGradeSheet(gsID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Grades&action=postGradesheetPreschool&gsID='+gsID);
}
