...
$xtpl->assign("IMPRESSIONS", $focus->impressions);
    ...
    $xtpl->assign("HIDE_FREQUENCY_IF_NEWSLETTER", "Select");
    $xtpl->assign("FREQUENCY_LABEL", $mod_strings['LBL_CAMPAIGN_FREQUENCY']);
    if((!isset($focus->frequency)) && (!isset($focus->id))){
        $xtpl->assign("FREQ_OPTIONS", get_select_options_with_id($app_list_strings['newsletter_frequency_dom'], 'Monthly'));
    }else{
        $xtpl->assign("FREQ_OPTIONS", get_select_options_with_id($app_list_strings['newsletter_frequency_dom'], $focus->frequency));
    }
