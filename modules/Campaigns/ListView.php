...
// if we are expecting to see list campaigns with type set to newsletter, then filter.. 
//.. the list view by type.  an example of this is if we are coming from ..
//.. the "newsletters" menu shortcut.
if (isset($_REQUEST['campaign_type']) and  $_REQUEST['campaign_type']=='newsletter'){
    $type = 'NewsLetter';
    if(strlen($where)>0){
      $where = "and campaigns.campaign_type = '$type'";
    }else{
        $where = "campaigns.campaign_type = '$type'";    
    }
}
//
