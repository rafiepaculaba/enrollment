    ...
    */
    function save($isUpdate){
        ...
        if(array_key_exists($this->module, $this->modules) && isset($this->bean->id)){
            ...
            if($isUpdate){
            	$whereClause = " WHERE id_c='" . $this->bean->id ."'";
                $query .= $whereClause;
                $querySelect = "SELECT id_c FROM {$this->bean->table_name}_cstm".$whereClause;
            }
            ...
            if(!$first){
                if(!$isUpdate){
                    ...
                    if((($this->db->dbType=='mysql' || $this->db->dbType=='mssql') && $this->db->getAffectedRowCount($result) == 0) || (($this->db->dbType=='oci8' ) && $this->db->getRowCount($result) == 0) ){
                    	//  13603 tyoung: getAffectedRowCount returns 0 in mysql when an update is issued with unchanged data. So we cannot rely on getAffectedRowCount to tell us if it is safe to perform an insert
                    	if ($this->db->getRowCount($this->db->query($querySelect)) == 0) {
    ...
    function addField($name,$label='', $type='Text',$max_size='255',$required_option='optional', $default_value='', $ext1='', $ext2='', $ext3='',$audited=0, $mass_update = 0 , $ext4='', $help='',$duplicate_merge=0){
        ...
        $label = $this->addLabel($label, $name);
    ...
    function addLabel($label, $name=''){
        ...
        $limit = 10;
        $count = 0;
        $field_key = $this->getDBName($name);
        $curr_field_key = $this->getDBName($name);
        ...
        LabelEditor::saveLabels(array('label_'. $curr_field_key=>$label),$this->module, $current_language);
