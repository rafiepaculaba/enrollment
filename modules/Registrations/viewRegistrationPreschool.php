...
require_once('modules/Registrations/RegistrationPreschool.php');
...
$accessCode = $access->getAccessCode("View Preschool Registration");
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    if (!$regID) {
        ...
        $accessCode = $access->getAccessCode("Edit Preschool Registration");
        $sugar_smarty->assign('hasEdit', $access->check_access($current_user->id, $accessCode) );
        
        // to check if the user has an access in delete
        $accessCode = $access->getAccessCode("Delete Preschool Registration");
        $sugar_smarty->assign('hasDelete', $access->check_access($current_user->id, $accessCode) );
        
        echo $sugar_smarty->fetch('modules/Registrations/templates/viewRegistrationPreschool.tpl');
    }
} else {
...
function deleteRegistration(regID)
{
    ...
    if (reply==true)
        redirect('index.php?module=Registrations&action=deleteRegistrationPreschool&regID='+regID);
}
</script>