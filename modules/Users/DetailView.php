...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME'], $mod_strings['LBL_MODULE_NAME'].": ".$focus->full_name." (".$focus->user_name.")", false);
echo "\n</p>\n";
global $theme;
 ...
 * get the user record
 */
$u = new User2($focus->id);
/**
 * get the usergroup of the user
 */
$ug = new UserGroup($u->groupID);
if ($ug->name)
    $xtpl->assign("USERGROUP", $ug->name);
    
//if (is_numeric($focus->department)) {
//    // get the equivalent 
//    $dept = new Department($focus->department);
//    $xtpl->assign("DEPARTMENT", $dept->deptCode);
//} else {
//    $xtpl->assign("DEPARTMENT", $focus->department);
//}

//$dept = new Department();
//$where[0]['deptCode'] = "='".$focus->department."'";
//$depts = $dept->retrieveAllDepartments($where);
//
//if ($depts) {
//	$theDepartment = $depts[0]['deptCode'];
//}
$xtpl->assign("DEPARTMENT", $focus->department);
...
if($sigDef = $focus->getPreference('signature_default')) {
	...
	$sig->retrieve($sigDef);
    $xtpl->assign('DEFAULT_SIGNATURE', $sig->name);
    /*    
...
$xtpl->assign('EMAIL_CHARSET', $locale->getPrecedentPreference('default_email_charset', $focus));
///////////////////////////////////////////////////////////////////////////////
////	INBOUND EMAIL
