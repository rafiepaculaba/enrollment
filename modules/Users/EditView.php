...
echo "\n<p>\n";
echo get_module_title($mod_strings['LBL_MODULE_NAME'], $mod_strings['LBL_MODULE_NAME'].": ".$focus->first_name." ".$focus->last_name."(".$focus->user_name.")", false);
echo "\n</p>\n";
global $theme;
...
$xtpl->assign('EXPORT_CHARSET', get_select_options_with_id($locale->availableCharsets, $locale->getExportCharset('', $focus)));
...
/**
 * display the list of usergroup to be included in the new user creation
 */
// ***********************************************************************************
    $ug = new UserGroup();
    $usergroups = $ug->retrieveAllUserGroups();
    $u = new User2($_GET['record']);
    $hasrecord = $_REQUEST['record'];
    
    
    $ugroups = '<select name="groupID" id="groupID" onchange="setAdmin();">'."\n";
    if ($usergroups) {
        $ugroups .= '<option value="">--------------------------------</option>'."\n";
        foreach ($usergroups as $group) {
            if ($u->groupID==$group['groupID']) {
                $ugroups .= '<option value="'.$group['groupID'].'" selected>'.$group['name'].'</option>'."\n";
            } else {
                $ugroups .= '<option value="'.$group['groupID'].'">'.$group['name'].'</option>'."\n";
            }
        }
    }
    $ugroups .= '</select>';
    ...
    // get all departments
    $dept = new Department();
    $departments = $dept->retrieveAllDepartments();
    
    $theDepartments = '<select name="department" id="department" >'."\n";
    if ($departments) {
        $theDepartments .= '<option value="">--------------------------------</option>'."\n";
        $theDepartments .= '<option value="Accounting"';
        
        if ($focus->department=="Accounting") {
            $theDepartments .= ' selected ';
        } 
        
        $theDepartments .= ' >Accounting</option>'."\n";
        $theDepartments .= '<option value="Registrar"';
        
        if ($focus->department=="Registrar") {
            $theDepartments .= ' selected ';
        }
        
        $theDepartments .= '>Registrar</option>'."\n";
        
        $theDepartments .= '<option value="High School"';
        
        if ($focus->department=="High School") {
            $theDepartments .= ' selected ';
        }
        
        $theDepartments .= ' >High School</option>'."\n";
        
        $theDepartments .= '<option value="Elementary"';
        
        if ($focus->department=="Elementary") {
            $theDepartments .= ' selected ';
        }
        
        $theDepartments .= ' >Elementary</option>'."\n";
        foreach ($departments as $d) {
            if ($focus->department==$d['deptCode']) {
                $theDepartments .= '<option value="'.$d['deptCode'].'" selected>'.$d['deptCode'].'</option>'."\n";
            } else {
                $theDepartments .= '<option value="'.$d['deptCode'].'">'.$d['deptCode'].'</option>'."\n";
            }
        }
    }
    $theDepartments .= '</select>';
    
    $ugtr = '
    <tr>
    	<td class="dataLabel"><slot>User Group <span class="required">*</span></slot></td>
    	<td  class="dataField"><slot>'.$ugroups.'</slot></td>
    	<td class="dataLabel"><slot>Department <span class="required">*</span></slot></td>
    	<td  class="dataField"><slot>'.$theDepartments.'</slot></td>
    </tr>';
    if(is_admin($current_user))
        $xtpl->assign('USERGROUP', $ugtr);
        
        $xtpl->assign('HASRECORD', $hasrecord);
...
////	LOCALE SETTINGS
////	Date/time format
$dformat = $timedate->get_date_format($focus);
$tformat = $timedate->get_time_format(true, $focus);
...
{
    if ($('groupID').value==1) {
        // this is admin user group id
        // set the admin option to true
        $('is_admin').disabled=false;
        $('is_admin').checked=true;
    } else {
        $('is_admin').disabled=true;
        $('is_admin').checked=false;
        
        // assign tabs to users
        if ($('hasrecord').value == '') {
            setTabs();
        }
...
function setTabs()
{
    for(i=0;i<$('hide_tabs_td').getElementsByTagName('select')[0].options.length;i++) {
        $('hide_tabs_td').getElementsByTagName('select')[0].options[i].selected=true;
    }
    SUGAR.tabChooser.right_to_left('display_tabs','hide_tabs', '10', '10', '');
    
    var hideMod = new Array();

//        Comment if the user group has access the module
    if ($('groupID').value=='2') {
        //Registrar
//        hideMod[0]  = "Home";
//        hideMod[1]  = "Students";
        hideMod[2]  = "Account";
//        hideMod[3]  = "Enrollments";
//        hideMod[4]  = "Departments";
//        hideMod[5]  = "Courses";
//        hideMod[6]  = "Subjects";
//        hideMod[7]  = "Schedules";
        hideMod[8]  = "Payments";
        hideMod[9]  = "Refunds";
//        hideMod[10] = "Curriculums";
//        hideMod[11] = "Credited";
//        hideMod[12] = "Grades";
        hideMod[13] = "SchoolFees";
//        hideMod[14] = "Reports";
    } else if ($('groupID').value=='3') {
        //Accounting
//        hideMod[0]  = "Home";
//        hideMod[1]  = "Students";
//        hideMod[2]  = "Account";
...
//  move to hidden tabs
    for(i=0;i<hideMod.length;i++) {
        if (hideMod[i]) {
            //$('display_tabs').options[i].selected=true;
//            alert($('display_tabs_td').getElementsByTagName('select')[0].options[i].value);
            $('display_tabs_td').getElementsByTagName('select')[0].options[i].selected=true;
        }
    }
    
    SUGAR.tabChooser.left_to_right('display_tabs','hide_tabs', '10', '10');
}
</script>