...
class Email extends SugarBean {
	...
	var $safe; // place holder for Safe_HTML libs
	...
	function mark_deleted($id) {
		...
		if(isset($sugar_config['email_default_delete_attachments']) && $sugar_config['email_default_delete_attachments'] == true) {
			$q = "SELECT id FROM notes WHERE parent_id = '{$this->id}'";
			$r = $this->db->query($q);
	...
	function removeAllNewlines($str) {
		...
		return str_replace($bad, $good, strip_tags(br2nl(from_html(from_html($str)))));
	...
	/**
	 * Overrides SugarBean::retrieve()
	 * Calls safe-ing code for display.
	 */
	function retrieve($id, $encoded=true, $deleted=true) {
		...
		if($ret) {
			...
			$ret->description_html = to_html($ret->safeText(from_html($ret->description_html)));
		}
...
*/
		...
		//////////////////////////////////////////////////////////////////////////
		////	ATTACHMENTS FROM Knowledge BASE
		if(isset($_REQUEST['kb_attachment']) && !empty($_REQUEST['kb_attachment'])) {
			$kb_removeArr = array();
			$kb_noteArray = array();
			
			if(isset($_REQUEST['temp_remove_kb_attachment']) && !empty($_REQUEST['temp_remove_kb_attachment'])) {
				$kb_removeArr = $_REQUEST['temp_remove_kb_attachment'];
			}
			

			foreach($_REQUEST['kb_attachment'] as $kb_doc_rev_id) {
				if(in_array($kb_doc_rev_id, $kb_removeArr)) {
					continue;
				}		
				...
				$docRev->retrieve($kb_doc_rev_id);
				
				
				
				// cn: bug 9723 - Emails with documents send GUID instead of Doc name
				$docNote->name = $docRev->filename;
				$docNote->filename = $docRev->filename;
				$docNote->description = $docRev->filename;
				$docNote->parent_id = $this->id;
				$docNote->parent_type = 'Emails';
				$docNote->file_mime_type = $docRev->file_mime_type;
				$docNote_id = $docNote->save();
				
				$this->saved_attachments[] = $docNote;
				
				$noteFile->duplicate_file($docRev->id, $docNote_id, $docRev->filename);
			}
			
		}
		////	END ATTACHMENTS FROM Knowledge BASE
		///////////////////////////////////////////////////////////////////////////
	...
	function getForwardHeader() {
		...
		$ret  = "{$mod_strings['LBL_FORWARD_HEADER']}<br /><br />";
		...
		$ret .= $this->replyDelimiter."{$mod_strings['LBL_SUBJECT']} {$this->name}<br />";
	 ...
	 * @return string
	 */
	function quoteHtmlEmail() {
		// TODO: extract all "real" text and apply "forwading" to the HTML
		//$text = trim(from_html($this->description_html));
		$text = trim(from_html(from_html($this->description_html)));
		...
		foreach($raw as $line) {
			...
			if($line != "") {
				//$out .= $this->replyDelimiter.to_html($line)."<br />";;
				$out .= $this->replyDelimiter.$line."<br />";;
	...
	function send() {
		...
		if( (isset($_REQUEST['setEditor']) /* from Email EditView navigation */
			...
			if(strpos($mail->Body, '"cid:')) {
				$matches = array();
				preg_match('/\"cid:.+?\"/i', $mail->Body, $matches);
				foreach($matches as $match) {
					$filename = str_replace(array('"', 'cid:'), array('',''), $match);
					$cid = $filename;
...
//<td valign="middle" height="30" class="listViewThS1" colspan="'.$colspan.'" id="hiddenhead" onClick="hideUserSelect();" onMouseOver="this.style.border = \'outset red 1px\';" onMouseOut="this.style.border = \'inset white 0px\';this.style.borderBottom = \'inset red 1px\';">
	...
	function save($check_notify = false) {
		// cn: bug 11158 - attributes have trailing whitespace and cause workflow to fail
		$this->to_addrs = trim($this->to_addrs);
		$this->from_addrs = trim($this->from_addr);
		$this->cc_addrs = trim($this->cc_addrs);
		$this->bcc_addrs = trim($this->bcc_addrs);
	...
	function fill_in_additional_list_fields() {
		...
		/*
		 * This code is now deprecated b/c of the standardization of Subpanel ListView code. 
		 * Sorting by date_modified results in non-sensical dates. 
	...
	///////////////////////////////////////////////////////////////////////////
   function moduleLoaded($moduleList,$moduleName){
		$loaded = false;         
