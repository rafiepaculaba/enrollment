...
if ($access->check_access($current_user->id,$accessCode)) {
    ...
    $semesters='<select name="semCode" id="semCode" onchange="selectSchedule();">'."\n";
	...
	echo $sugar_smarty->fetch('modules/SchoolFees/templates/assignLabFee.tpl');
} else {
...
<script>
addToValidate('frmLabFee','schYear', '', true, 'School Year');
addToValidate('frmLabFee','semCode', '', true, 'Semester');
addToValidate('frmLabFee','schedID', '', true, 'Sched Code');
...
function selectSchedule()
{
    ...
    if ($('schYear').value != '' && $('semCode').value != '') {
        ...
        get_data="schYear=" + $('schYear').value + "&semCode=" + $('semCode').value + "&action=selectschedulecol";
        ajaxQuery("modules/SchoolFees/schoolfeeHandler.php",'GET',get_data,"","selectScheduleHandle");
    } else if ($('schYear').value == '' || $('semCode').value == '') {
...
{
    if ($('schYear').value != '' && $('semCode').value != '') {
        get_data="schedID=" + $('schedID').value + "&action=displayschedulecol";
...
if (xmlHttp.readyState==4) {
    	...
    	if (ret) {
			...
			$('courseCode').value 		= myData[0].courseCode;
			$('curName').value 			= myData[0].curName;
