...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	echo $sugar_smarty->fetch('modules/SchoolFees/templates/createSchoolFeeHS.tpl');
} else {
...
function checkDuplicate()
{
    if (check_form('frmSchoolFee')) {
        get_data="schYear=" + $('schYear').value + "&yrLevel=" + $('yrLevel').value + "&item=" + $('item').value + "&action=checkduplicatehs";
...
function checkDuplicateHandle() 
{
	if (xmlHttp.readyState==4) {
    	...
    	if (ret=='') {
    		redirect('index.php?module=SchoolFees&action=createSchoolFeeHS&feeID=<?php echo $_GET['feeID']; ?>'); // redirect to entry page
    	} else {
	    	if (ret==-1) {
	    		// School Fee valid
                $('frmSchoolFee').submit();
