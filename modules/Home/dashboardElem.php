...
echo "\n<p>\n";
echo get_module_title("view_inline", $mod_strings['LBL_MODULE_ELEM_DASHBOARD'], false);
echo "\n</p>\n";
global $theme;
global $esConfig;
...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	$config = new Config();
        ...
        $sugar_smarty->assign('RESULT', $reportClass->generateEnrollmentStatusElem($result,$total));
		 ...
		 */
//		$query = "Select distinct form137.idno from form137";
//		try {
...
//			
//			$ttl_form137_entry  = count($records);
//			
//		} catch (PDOException $e) {
//		    echo "SQL query error.";
//		}
//		
//		// this will check if the form137 is empty becuase it will not work during checking with sub query
//		if ($ttl_form137_entry) {
//	       $query = "SELECT count(distinct enrollments.idno) ttl_new FROM enrollments, form137 where enrollments.rstatus>0 and enrollments.schYear='$schYear' and (enrollments.idno not in (select distinct form137.idno from form137))";
//		} else {
//	       $query = "SELECT count(distinct idno) ttl_new FROM enrollments where enrollments.schYear='$schYear' and rstatus>0";
//		}
	    ...
	    $query = "select sum(amount) total from registration_payments where schYear='$schYear' ";
//	    $query = "select sum(amount) total from registration_payments where schYear='$schYear' and rstatus=1";
    ...
    echo $sugar_smarty->fetch('modules/Home/templates/dashboardElem.tpl');	
} else {
