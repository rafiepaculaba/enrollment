...
$accessCode = $access->getAccessCode("View HS Refund");
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if($refund->rstatus == 1) {
	    ...
	    $accessCode = $access->getAccessCode("Cancel HS Refund");
	    $sugar_smarty->assign('hasCancel', $access->check_access($current_user->id, $accessCode) );
    } 

	echo $sugar_smarty->fetch('modules/Refunds/templates/viewRefundHS.tpl');
	} else {
...
function cancelRefund(refundID, accID, amount)
{
    ...
    if (reply==true)
        redirect('index.php?module=Refunds&action=cancelRefundHS&refundID='+refundID+'&accID='+accID+'&amount='+amount);
}
</script>