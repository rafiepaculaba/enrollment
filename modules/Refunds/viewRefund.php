...
$accessCode = $access->getAccessCode("View Col Refund");
if ($access->check_access($current_user->id,$accessCode)) {

	$refundID = $_GET['refundID'];
	$refund = new Refund($refundID);
	switch ( $refund->semCode ) 
	{
		case 1:
			$semCode="1st Sem"	;
			break;
	...
	if($refund->rstatus == 1) {
	    ...
	    $accessCode = $access->getAccessCode("Cancel Col Refund");
	    $sugar_smarty->assign('hasCancel', $access->check_access($current_user->id, $accessCode) );
    } 

	echo $sugar_smarty->fetch('modules/Refunds/templates/viewRefund.tpl');
	} else {
...
function cancelRefund(refundID, accID, amount)
{
    ...
    if (reply==true)
        redirect('index.php?module=Refunds&action=cancelRefund&refundID='+refundID+'&accID='+accID+'&amount='+amount);
}
</script>