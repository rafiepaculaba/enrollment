...
if ($access->check_access($current_user->id,$accessCode)) {
	...
	if ($_GET['cmdFilter']) {
		...
		$refundID     	= $_SESSION[$_GET['module'].'Col_refundID'];
	...
	$_SESSION[$_GET['module'].'Col_refundID']	= $refundID;
	...
	if ($semCode) {
        if (count($conds[0])) {
            $conds[0][' AND refunds.semCode'] = " = '$semCode' ";
        } else {
            $conds[0]['refunds.semCode'] = " = '$semCode' ";
        }
    }
	if (trim($refundID)!='') {
        if (count($conds[0])) {
...
*/		
	...
	$main_url="index.php?module=Refunds&action=listRefunds&refundID=$refundID&schYear=$schYear&semCode=$semCode&idno=$idno&lname=$lname&fname=$fname&mname=$mname&rstatus=$rstatus";
	...
	$sugar_smarty->assign('amount', $amount );
	...
	echo $sugar_smarty->fetch('modules/Refunds/templates/listRefunds.tpl');
