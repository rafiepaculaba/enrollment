...
class DBHelper
{
	...
	*/
	function oneColumnSQLRep($fieldDef,  $ignoreRequired = false, $table=''){
        ...
        if($sugar_config['dbconfig']['db_type'] == 'oci8') {
        	...
        	$required = "";
        	if($ignoreRequired == false && isset($fieldDef['required'])) {

		        $required = 'NULL';  
		
		        if (array_key_exists('required',$fieldDef) && ($fieldDef['required'] == true || $fieldDef['required'] == 'true' || $fieldDef['required'] == 1)){
					 $required =  "NOT NULL";
		        }
		        
		        if($name == 'id' && !array_key_exists('required',$fieldDef)) {
		        	$required =  "NOT NULL";
		        }
        ...
        else
        {
	        ...
	        if ($ignoreRequired == false && (array_key_exists('required',$fieldDef) && ($fieldDef['required'] == true || $fieldDef['required'] == 'true' || $fieldDef['required'] == 1))){
				 $required =  "NOT NULL";
	        }
	        if($name == 'id' && !array_key_exists('required',$fieldDef)) {
	        	$required =  "NOT NULL";
	...
	*/
	/*
	 * jc: deprecated - this method should be overridden in child classes because
	 * of the specific alter table syntax per each rdbms
	 */
	...
	*/
	function alterColumnSQL($tablename, $newFieldDefs){
       return $this->changeColumnSQL($tablename, $newFieldDefs, 'modify', false);
	}
