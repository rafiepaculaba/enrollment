...
class MssqlManager2 extends DBManager
{
    var $dbType = 'mssql';
...
function getTableNameFromModuleName($module_str,$sql){
	...
	*/
    ...
    function appendN($sql) {
	    ...
	    $replace = array();
	    if (!empty($matches)) {
	    	//$GLOBALS['log']->debug($matches[0]);
	        foreach ($matches[0] as $key=>$value) {
	        	       //$GLOBALS['log']->fatal($value . "is_numeric=" . is_numeric(trim(str_replace($this->str_replace, "", $value))));
	                   //$GLOBALS['log']->fatal("strlen=" . strlen($value));
	                   ...
	                   if (!empty($value) && !is_numeric(trim(str_replace($this->replacements, "", $value)))) {
