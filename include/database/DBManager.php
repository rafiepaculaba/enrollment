...
class DBManager
{
	...
	*/
    function repairTable($bean, $execute = true){
        ...
        $stuff = $this->repairTableParams($tablename, $fielddefs,$indices, $execute);
    ...
    function repairTableParams($tablename,  $fielddefs,$indices, $execute = true){
        ...
        foreach($fielddefs as $value){
            ...
            switch($result['msg']){
                ...
                case 'no_match':
                	//jc: 4.5.1 fix ensues
                    //in oracle, a column that is of type clob cannot be altered in the traditional sense. so if we
                    //are trying to make a change to a clob column, we must add a new clob column with the modifications
                    //then copy the appropriate data into the new column, drop the old column and then rename the new
                    //column
					if($sugar_config['dbconfig']['db_type'] == 'oci8' && ($result['table1']['Type'] == 'clob' || $result['table2']['Type'] == 'clob'))
					{
						//null, 'null', and '' are equivalent for all intents and purposes in this case
						$t1_null = $result['table1']['Default'] == 'null' || $result['table1']['Default'] == "'null'" || $result['table1']['Default'] == "" || $result['table1']['Default'] == null;
						$t2_null = $result['table2']['Default'] == 'null' || $result['table2']['Default'] == "'null'" || $result['table2']['Default'] == "" || $result['table1']['Default'] == null;
                    ...
                    if(strtoupper($result['table1']['Null']) == strtoupper($result['table2']['Null'])) {
                    	unset($value['required']);
                    }
                    
                    //jc: if we are setting a column to not null and there is not default, when data is present
                    //the statement will get kicked back.
                    if(strtoupper($result['table1']['Null']) == 'N' && $result['table1']['Null'] === $result['table2']['Null'] && (!isset($value['Default'])))
                    {
                    	if(array_key_exists('Default', $value)) unset($value['Default']);
                    	if(array_key_exists('required', $value)) unset($value['required']);
                    }
		...
		//jc: just in case we make it here without having created a repair_table (in the case of
		//an error on table create due to a bad vardef, etc)
		if($this->tableExists('repair_table'))
        	$this->dropTableName('repair_table');
	...
	//jc: describeIndex has been moved into the db specific manager because it is a db specific operation
		...
		if(!empty($count_query))
{
		...
		if(isset($bean->custom_fields)){
			...
			$custom_fields_list = array_keys($bean->custom_fields->getAvailableFields(true));
			
			foreach($custom_fields_list as $k => $v){
					...
					}else
{
						...
						if($db_type == 'mysql' && empty($val) && ($type == 'datetime' ||  $type == 'date' || $type == 'int')){
							...
							if(!empty($custom_fields[$fieldDef['name']]))
								...
								$cstm_values[$fieldDef['name']] = (!empty($val) ? "'".$val."'" : '0');
							
							else
								
								$values[$fieldDef['name']] = (!empty($val) ? "'".$val."'" : '0');
							...
							if(!empty($custom_fields[$fieldDef['name']])){
								...
								$cstm_values[$fieldDef['name']] = $dbHelper->massageValue($val, $fieldDef);
