...
class MassUpdate
{
	...
	function getMassUpdateFormHeader($multi_select_popup = false)
	{
		...
		$tempString = '<script type="text/javascript" src="include/javascript/popup_parent_helper.js?s=' . $sugar_version . '&c=' . $sugar_config['js_custom_version'] . '"></script>'
		...
		if($_REQUEST['module'] == 'Emails') {
			...
			$type = (isset($_REQUEST['ie_type'])) ? $_REQUEST['ie_type'] : 'inbound';
			// determine owner
			$tempString .=<<<eoq
...
eoq;
	...
	function handleMassUpdate(){
		...
		if(!is_array($this->sugarbean) && $this->sugarbean->bean_implements('ACL') &&
	...
	function getMassUpdateForm(){
		...
		if($this->sugarbean->ACLAccess('Delete', true) && $this->sugarbean->object_name != 'User' && $this->sugarbean->object_name != 'Employee') {
