...
function return_module_language($language, $module) {
	...
	// If we are in debug mode for translating, turn on the prefix now!
	if(isset($sugar_config['translation_string_prefix']) && !empty($sugar_config['translation_string_prefix'])) {
...
function installed_sugarmodule($module_name){
	$db = &PearDatabase::getInstance();

	$query = "SELECT id FROM versions WHERE name='" .$module_name. "'";
