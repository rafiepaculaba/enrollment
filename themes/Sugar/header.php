...
require_once('include/calendarSetup.php');
...
$xtpl->assign("CURRENT_MODULE_NAME", $_GET['module']);
$xtpl->assign("CURRENT_ACTION_NAME", $_GET['action']);
$xtpl->assign("DATE", date("Y-m-d"));
$xtpl->assign("TITLE", $app_strings['LBL_SEARCH']);

/**
 * blumango codes: not to display the header if in login mode
 */
$xtpl->assign("LOGIN_ERROR",  $_SESSION['login_error']);
...
/**
 * description: this will control the header navigation
 *              no header nav if login form
 */
$header_nav = "";



if ($currentModule!="Users" && $_GET['action']!="Login") {
    $xtpl->assign("HEADER_NAV", $header_nav);
}

////////////////////////// END OF HEADER NAV CONTROLER /////////////////////////
