...
function get_field_list(&$value){
	...
	if(!empty($value->field_defs)){
		foreach($value->field_defs as $var){
			...
			if(!empty($var['custom_type'])){

				$var['type'] = $var['custom_type'];
				//don't support multi enum through portal
				if(strcmp($var['type'],  'multienum') ==0)continue;
	 		}
	...
	if(!empty($value->field_defs)){
		...
		foreach($value->field_defs as $var){
			...
			if(isset($value->$var['name'])){
				$val = $value->$var['name'];
				$type = (empty($var['custom_type']))?$var['type']: $var['custom_type'];
				
				if(strcmp($type, 'date') == 0){
					$val = substr($val, 0, 10);
				}elseif((strcmp($type, 'enum') == 0) && !empty($var['options'])){
					global $app_list_strings;
					if(!empty($app_list_strings[$var['options']]) && !empty($app_list_strings[$var['options']][$val])){
...
function check_for_duplicate_contacts(&$seed){
	...
	//rrs bug 15327
	$query .=" AND assigned_user_id = '". $current_user->id ."'";
