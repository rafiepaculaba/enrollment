 ...
 ********************************************************************************/
$portal_modules = array('Contacts', 'Accounts', 'Cases', 'Bugs', 'Notes', 'KBDocuments');
/*
BUGS
*/
...
function portal_get_entry_list_limited($session, $module_name, $where, $order_by, $select_fields, $row_offset, $limit) {
			...
			if ($module_name == 'Accounts') {
				...
				if ($module_name == 'Bugs') {
					...
					//_ppd($list);
						...
						$GLOBALS['log']->error("no_module_support");
	...
	return array (
		'result_count' => sizeof($output_list), 'next_offset' => 0, 'field_list' => $field_list, 'entry_list' => $output_list, 'error' => $error->get_soap_array());
}
...
if(!empty($GLOBALS['beanList']['KBDocuments'])){
	$valid_modules_for_contact['KBDocuments'] = 1;	
}
?>
